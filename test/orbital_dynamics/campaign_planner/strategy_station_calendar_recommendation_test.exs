...
defmodule OrbitalDynamics.CampaignPlanner.StrategyStationCalendarRecommendationTest do
  ...
  test "strategy recommendation skips unavailable station-calendar replay by default" do
    prior_plan =
      base_plan(%{
    ...
    unavailable_report = station_calendar_report(:unavailable, :unavailable_station_calendar)
    reserved_report = station_calendar_report(:reserved, :reserved_station_calendar)
    ...
    blocked_artifact =
      strategy(prior_plan,
        mission_state:
          ...
          |> Map.put("source_station_calendar_report", unavailable_report),
        strategy_policy: %{
        ...
        branches: [
          %{id: "baseline"},
          %{id: "unavailable_station_calendar", events: [urgent_target_event]}
        ],
        current_epoch_s: 0.0
      )

    blocked_branch = branch(blocked_artifact, "unavailable_station_calendar")
    ...
    assert "station_calendar_unavailable_blocked" in blocked_risk_types

    assert Enum.any?(
             blocked_branch["risk_indicators"],
             &(&1["type"] == "station_calendar_pressure" and
                 &1["feedback_scope"] == "station_calendar" and
                 &1["affected_contact_availability_counts"] == %{"unavailable" => 1})
           )
    ...
    review_artifact =
      strategy(prior_plan,
        mission_state:
          mission_state_with_refresh_inputs()
          |> Map.put(:objectives, [%{"type" => "priority_commitment", "target_id" => "target_a"}])
          |> Map.put("source_station_calendar_report", reserved_report),
        strategy_policy: %{
          "mission_value_weight" => 10.0,
          "risk_weight" => 0.0,
          "approval_load_weight" => 0.0
        ...
        branches: [
          %{id: "baseline"},
          %{id: "reserved_station_calendar", events: [urgent_target_event]}
        ],
        current_epoch_s: 0.0
      )

    review_branch = branch(review_artifact, "reserved_station_calendar")

    assert review_artifact["recommendation"]["recommended_branch_id"] ==
             "reserved_station_calendar"
    ...
    assert Enum.any?(
             ...
             &(&1["type"] == "station_calendar_pressure" and
                 &1["feedback_scope"] == "station_calendar" and
                 &1["affected_contact_availability_counts"] == %{"reserved" => 1})
           )
  ...
  defp station_calendar_report(station_status, boundary) do
    OrbitalDynamics.station_calendar_report(
      ...
      source: "campaign_planner_test.#{boundary}"
    )
