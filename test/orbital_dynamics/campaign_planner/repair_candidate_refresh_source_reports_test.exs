...
defmodule OrbitalDynamics.CampaignPlanner.RepairCandidateRefreshSourceReportsTest do
  ...
  test "repair can use candidate_refresh.v1 candidates instead of stale prior candidates" do
    ...
    source_candidate_refresh_assumptions = %{
      ...
      "candidate_limit_policy" => %{"max_candidates" => 4},
      "propagator" => "Elixir.OrbitalDynamics.Propagators.TwoBody",
    ...
    canonical_timeline_activity_precondition_summary =
      source_timeline_activity_precondition_summary
    ...
    canonical_timeline_activity_lifecycle_state =
      source_timeline_activity_lifecycle_state
      |> :json.encode()
      |> IO.iodata_to_binary()
      |> String.replace("cmd_provider", "cmd_canonical_lifecycle")
      |> :json.decode()
    ...
    canonical_timeline_activity_state =
      source_timeline_activity_state
      |> :json.encode()
      |> IO.iodata_to_binary()
    ...
    artifact =
      repair(
        ...
        candidate_refresh:
          ...
          |> Map.put("source_timeline_activity_state", [source_timeline_activity_state])
          |> Map.put("timeline_activity_state", canonical_timeline_activity_state)
          |> Map.put(
            "source_timeline_activity_status_state",
            [source_timeline_activity_status_state]
          )
    ...
    assert artifact["source_contact_allocation_provider_reservation_request_summaries"] == [
             source_contact_allocation_provider_reservation_request_summary
           ]
    ...
    assert artifact["source_contact_allocation_reservation_conflict_summaries"] == [
             source_contact_allocation_reservation_conflict_summary
           ]
    ...
    assert artifact["source_contact_allocation_capacity_pack_summaries"] == [
             source_contact_allocation_capacity_pack_summary
           ]

    assert artifact["source_contact_allocation_summary"] ==
             source_contact_allocation_summary

    assert artifact["source_contact_allocation_summaries"] == [
             source_contact_allocation_summary
           ]
    ...
    assert artifact["source_timeline_activity_precondition_summaries"] == [
             source_timeline_activity_precondition_summary,
             canonical_timeline_activity_precondition_summary
           ]
    ...
    assert %{
             ...
             "source" =>
               "campaign_repair.source_timeline_activity_precondition_summaries[0].summary",
             "subject_id" => "timeline:cmd_source",
    ...
    assert %{
             "source" =>
               "campaign_repair.source_timeline_activity_precondition_summaries[1].summary",
             "timeline_id" => "timeline:cmd_canonical",
    ...
    assert artifact["source_timeline_activity_lifecycle_states"] == [
             source_timeline_activity_lifecycle_state,
             canonical_timeline_activity_lifecycle_state
           ]
    ...
    assert %{
             "review_type" => "timeline_lifecycle_state_review",
             "source" => "campaign_repair.source_timeline_activity_lifecycle_states[0].state",
             "subject_id" => "timeline:cmd_provider",
             "timeline_id" => "timeline:cmd_provider",
             "activity_id" => "cmd_provider",
             "planned_activity_id" => "cmd_provider",
             ...
             "required_operator_action" => "review_activity_approval",
             "approval_status" => "operator_review_required",
             "source_timeline_lifecycle_state" => ^source_timeline_activity_lifecycle_state
           } =
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] ==
                   "campaign_repair.source_timeline_activity_lifecycle_states[0].state")
             )

    assert %{
             "source" => "campaign_repair.source_timeline_activity_lifecycle_states[1].state",
             "timeline_id" => "timeline:cmd_canonical_lifecycle",
             "activity_id" => "cmd_canonical_lifecycle",
             "source_timeline_lifecycle_state" => ^canonical_timeline_activity_lifecycle_state
           } =
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] ==
                   "campaign_repair.source_timeline_activity_lifecycle_states[1].state")
             )
    ...
    assert artifact["source_timeline_activity_states"] == [
             source_timeline_activity_state,
             canonical_timeline_activity_state,
             source_timeline_activity_status_state,
             source_timeline_activity_approval_state
    ...
    assert %{
             "review_type" => "timeline_lifecycle_state_review",
             "source" => "campaign_repair.source_timeline_activity_states[0].state",
             "subject_id" => "timeline:cmd_lock",
             "timeline_id" => "timeline:cmd_lock",
             "activity_id" => "cmd_lock",
             "source_timeline_activity_state" => ^source_timeline_activity_state
           } =
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] == "campaign_repair.source_timeline_activity_states[0].state")
             )

    assert %{
             "source" => "campaign_repair.source_timeline_activity_states[1].state",
             "timeline_id" => "timeline:cmd_canonical_state",
             "activity_id" => "cmd_canonical_state",
             "source_timeline_activity_state" => ^canonical_timeline_activity_state
           } =
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] == "campaign_repair.source_timeline_activity_states[1].state")
             )

    assert %{
             "source" => "campaign_repair.source_timeline_activity_states[2].state",
             ...
             "source_timeline_lifecycle_state" => ^source_timeline_activity_status_state
           } =
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] == "campaign_repair.source_timeline_activity_states[2].state")
             )

    assert %{
             "source" => "campaign_repair.source_timeline_activity_states[3].state",
             "timeline_id" => "timeline:cmd_approval_source",
             "activity_id" => "cmd_approval_source",
             "transition_decision" => "review",
             "required_operator_action" => "review_activity_approval",
             "source_timeline_lifecycle_state" => ^source_timeline_activity_approval_state
           } =
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] == "campaign_repair.source_timeline_activity_states[3].state")
             )
    ...
    assert artifact["source_timeline_transition_application_report"] ==
             source_timeline_transition_application_report
    ...
    assert %{
             "contact_allocation_review_count" => 13,
             ...
             "station_reservation_contact_ids_by_match_status" => %{
               ...
               "overlap" => ["dl_3", "dl_reserved_intruder"]
             },
             "station_reservation_status_counts" => %{"confirmed" => 2, "reserved" => 1},
             "station_reservation_contact_ids_by_status" => %{
               "confirmed" => ["dl_reserved_intruder", "dl_reserved_owner"],
               "reserved" => ["dl_3"]
             },
             "station_reserved_by_counts" => %{"network_partner" => 1, "ops_team_b" => 2},
             "station_reservation_contact_ids_by_reserved_by" => %{
               "network_partner" => ["dl_3"],
    ...
    assert %{
             ...
             "source" => "campaign_repair.source_contact_allocation_summaries[0].review_rows",
             "contact_id" => "dl_2",
             "ground_station_id" => "equator_prime",
             ...
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] ==
                   "campaign_repair.source_contact_allocation_summaries[0].review_rows" and
                   &1["contact_id"] == "dl_2")
             )
    ...
    assert %{
             "review_type" => "contact_allocation_review",
             "source" =>
               "campaign_repair.source_contact_allocation_capacity_pack_summaries[0].review_rows",
             ...
             Enum.find(
               artifact["operator_review_package"]["rows"],
               &(&1["source"] ==
                   "campaign_repair.source_contact_allocation_capacity_pack_summaries[0].review_rows" and
    ...
    assert %{
             ...
             "source" =>
               "campaign_repair.source_contact_allocation_capacity_pack_summaries[0].reduced_capacity_pack_groups",
    ...
    assert %{
             "review_type" => "contact_allocation_review",
             "source" =>
               "campaign_repair.source_contact_allocation_reservation_conflict_summaries[0].reservation_review_rows",
    ...
    assert %{
             "review_type" => "contact_allocation_review",
             "source" =>
               "campaign_repair.source_contact_allocation_provider_reservation_request_summaries[0].provider_reservation_request_rows",
  ...
  test "repair preserves every capacity-pack source summary without double-counting its mirror" do
    summary =
      "study_results/contact_allocation_capacity_pack_summary_v1.json"
      |> File.read!()
      |> :json.decode()
    ...
    candidate_refresh =
      candidate_refresh_artifact([], [])
      |> Map.put("source_contact_allocation_capacity_pack_summary", [summary, summary])
      |> Map.put("contact_allocation_capacity_pack_summary", summary)
    ...
    assert artifact["source_contact_allocation_capacity_pack_summary"] == summary

    assert artifact["operator_review_package"][
             "contact_allocation_capacity_pack_review_count"
           ] == 3

    for index <- 0..2 do
      source_prefix =
        "campaign_repair.source_contact_allocation_capacity_pack_summaries[#{index}]"
  ...
  test "repair preserves every reservation-conflict source summary without double-counting its mirror" do
    summary =
      "study_results/contact_allocation_reservation_conflict_summary_v1.json"
      |> File.read!()
      |> :json.decode()
    ...
    candidate_refresh =
      candidate_refresh_artifact([], [])
      |> Map.put("source_contact_allocation_reservation_conflict_summary", [summary, summary])
      |> Map.put("contact_allocation_reservation_conflict_summary", summary)
    ...
    assert artifact["source_contact_allocation_reservation_conflict_summary"] == summary
    assert artifact["operator_review_package"]["contact_allocation_review_count"] == 3

    for index <- 0..2 do
      source_prefix =
        "campaign_repair.source_contact_allocation_reservation_conflict_summaries[#{index}]"
    ...
    legacy_artifact =
      artifact
      |> Map.delete("source_contact_allocation_reservation_conflict_summaries")
      |> Map.delete("operator_review_package")
      |> Map.delete("cadence_import_manifest")
  ...
  test "repair preserves every provider-reservation-request source summary without double-counting its mirror" do
    summary =
      "study_results/contact_allocation_provider_reservation_request_summary_v1.json"
      |> File.read!()
      |> :json.decode()
    ...
    candidate_refresh =
      ...
      |> Map.put("contact_allocation_provider_reservation_request_summary", summary)
    ...
    assert artifact["source_contact_allocation_provider_reservation_request_summary"] == summary
    assert artifact["operator_review_package"]["contact_allocation_review_count"] == 6

    for index <- 0..2 do
      source_prefix =
        "campaign_repair.source_contact_allocation_provider_reservation_request_summaries[#{index}]"
  ...
  test "repair preserves every compact allocation source summary without double-counting its mirror" do
    summary =
      "study_results/contact_allocation_summary_v1.json"
      |> File.read!()
      |> :json.decode()
    ...
    candidate_refresh =
      candidate_refresh_artifact([], [])
      |> Map.put("source_contact_allocation_summary", [summary, summary])
      |> Map.put("contact_allocation_summary", summary)
    ...
    assert artifact["source_contact_allocation_summary"] == summary
    assert artifact["operator_review_package"]["contact_allocation_review_count"] == 9

    for index <- 0..2 do
      source_prefix = "campaign_repair.source_contact_allocation_summaries[#{index}]"
    ...
    legacy_artifact =
      artifact
      |> Map.delete("source_contact_allocation_summaries")
      |> Map.delete("operator_review_package")
      |> Map.delete("cadence_import_manifest")
