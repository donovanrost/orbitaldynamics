defmodule OrbitalDynamics.CandidateRefresh.DownlinkFeedbackTimelineFeedbackBuildTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.{CandidateRefresh, Epoch, ResultSet, Schema, TimelineFeedback}

  test "applies station downlink demand derived from realized timeline feedback" do
    feedback =
    ...
    assert get_in(feedback, ["operational_feedback", "downlink_demand_mb"]) == %{
             "equator_prime" => 240.0
           }

    assert "timeline_feedback.contact.required_downlink_mb:dl_equator_prime" in get_in(
             feedback,
    ...
    assert "timeline_feedback.contact.required_downlink_mb:dl_equator_prime" in downlink[
  ...
  test "applies operational feedback directly from a source timeline feedback report" do
    feedback_report =
      TimelineFeedback.reconcile(
        [
          %{
        ...
        [
          %{
            ...
            status: :partial,
            actual_throughput_mb: 120.0,
            trust_boundary: :cadence_realized_feedback
          }
        ]
      )
