defmodule OrbitalDynamics.CandidateRefresh.FreshnessBuildTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.{CandidateRefresh, Epoch, ResultSet, Schema}

  test "warns when freshness policy marks the accepted state or horizon stale" do
    refresh =
      refresh_request()
      |> put_in(["freshness_policy"], %{
        "max_snapshot_age_s" => 60.0,
        "max_horizon_start_offset_s" => 10.0
      })
    ...
    artifact =
      ...
      |> CandidateRefresh.build(
        ...
        generated_at: ~U[2026-05-14 00:02:00Z]
      )

    assert %{
             "schema_contract" => "freshness_report.v1",
             "accepted_snapshot_age_s" => 120.0,
  ...
  test "marks refresh stale when accepted state quality is below policy" do
    refresh =
      refresh_request()
      |> put_in(["accepted_planning_state", "quality"], %{"level" => "draft"})
      |> put_in(["freshness_policy"], %{
    ...
    assert %{
             "accepted_state_quality_level" => "draft",
             "allowed_state_quality_levels" => ["accepted", "planning_accepted"],
             "state_quality_status" => "not_accepted",
             "status" => "stale",
             "stale_reasons" => ["accepted_state_quality_below_policy"],
             "unknown_reasons" => []
           } = artifact["freshness_report"]
