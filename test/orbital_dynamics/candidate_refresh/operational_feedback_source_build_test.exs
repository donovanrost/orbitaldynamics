defmodule OrbitalDynamics.CandidateRefresh.OperationalFeedbackSourceBuildTest do
  ...
  test "preserves declared operational feedback trust boundary in refresh provenance" do
    artifact =
      result_set()
      |> CandidateRefresh.build(
        candidate_refresh:
          ...
          |> put_in(["operational_feedback"], %{
            ...
            "provenance" => %{"source" => "cadence_feedback_snapshot"},
  ...
  test "uses mission-state embedded operational feedback when top-level feedback is absent" do
    artifact =
      result_set()
      |> CandidateRefresh.build(
        candidate_refresh:
    ...
    assert [downlink] = Enum.filter(artifact["candidate_activities"], &(&1["type"] == "downlink"))
    assert downlink["contact_success_factor"] == 0.25
  ...
  test "top-level operational feedback overrides mission-state embedded feedback" do
    artifact =
      ...
      |> CandidateRefresh.build(
        candidate_refresh:
          ...
          |> Map.put("mission_state", %{
            "operational_feedback" => %{
              "trust_boundary" => "mission_state_feedback",
              "contact_success_rate" => %{"equator_prime" => 0.25}
            }
          })
          |> Map.put("operational_feedback", %{
            "trust_boundary" => "request_override_feedback",
            "contact_success_rate" => %{"equator_prime" => 0.75}
          }),
        generated_at: ~U[2026-05-14 00:00:00Z]
      )
    ...
    assert [downlink] = Enum.filter(artifact["candidate_activities"], &(&1["type"] == "downlink"))
    assert downlink["contact_success_factor"] == 0.75

    assert {:ok, %{"schema_contract" => "candidate_refresh.v1", "status" => "pass"}} =
             Schema.validate_artifact(artifact)
  end

  test "uses accepted-planning-state embedded operational feedback when request feedback is absent" do
    artifact =
      result_set()
      |> CandidateRefresh.build(
        candidate_refresh:
          refresh_request()
          |> put_in(["accepted_planning_state", "operational_feedback"], %{
            "trust_boundary" => "accepted_state_feedback",
    ...
    assert %{
             "operational_feedback" => %{
               "trust_boundary" => "accepted_state_feedback",
               "source_path" => "accepted_planning_state.operational_feedback",
