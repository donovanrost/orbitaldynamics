defmodule OrbitalDynamics.CandidateRefresh.TimelineDiffFeedbackObservationCommandBuildTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.{CandidateRefresh, Epoch, ResultSet, Schema, Timeline}

  test "derives target revisit objectives from changed observation failure rows in source timeline diff reports" do
    diff_report =
      Timeline.diff_report(
        [
          %{
            ...
            metadata: %{timeline_id: :"timeline:obs_changed"}
          }
        ],
        [
          %{
      ...
      |> update_in(["rows"], fn rows ->
        Enum.map(rows, fn row ->
          ...
          |> Map.put("replacement_activity_type", "Imaging")
        end)
      end)
  ...
  test "derives observation quality feedback from changed observation rows in source timeline diff reports" do
    diff_report = %{
      ...
      "rows" => [
        %{
          ...
          "replacement_activity_type" => "imaging",
          ...
          "replacement_activity_context" => %{
            ...
            "image_quality_source" => "ops_image_assessment",
    ...
    assert observe["image_quality_score"] == 0.35
    assert observe["image_quality_status"] == "marginal"
    assert observe["image_quality_source"] == "ops_image_assessment"
    assert observe["cloud_cover_fraction"] == 0.65
    assert observe["blur_score"] == 0.2
    assert observe["observation_success_factor"] == 0.35
  ...
  test "derives command success feedback from changed command failure rows in source timeline diff reports" do
    diff_report =
      Timeline.diff_report(
        [
          %{
      ...
      |> update_in(["rows"], fn rows ->
        Enum.map(rows, fn row ->
          row
          |> Map.put("diff_status", "Changed")
          |> Map.put("replacement_activity_type", "Health Check")
        end)
      end)
