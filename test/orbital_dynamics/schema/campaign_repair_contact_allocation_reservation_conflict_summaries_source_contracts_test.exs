defmodule OrbitalDynamics.Schema.CampaignRepairContactAllocationReservationConflictSummariesSourceContractsTest do
  ...
  test "validates every ordered V2 source reservation-conflict summary", %{
    artifact: artifact
  } do
  ...
  test "rejects indexed source drift and a non-list plural shape", %{artifact: artifact} do
    invalid_count =
      put_in(artifact, [@plural_field, Access.at(1), "reservation_conflict_contact_count"], 99)

    assert {:error, count_report} = Schema.validate_artifact(invalid_count)

    assert Enum.any?(
             count_report["errors"],
             &(&1["path"] == "$.#{@plural_field}[1].reservation_conflict_contact_count")
           )
  ...
  test "exports the plural source reservation-conflict property" do
    assert {:ok, schema} = Schema.json_schema("campaign_repair.v2")
    assert get_in(schema, ["properties", @plural_field, "type"]) == "array"

    assert get_in(
             schema,
             ["$defs", "contact_allocation_reservation_conflict_summary.v1", "type"]
           ) == "object"
  end
