...
defmodule OrbitalDynamics.Schema.CampaignRepairSourceProviderCounterofferReviewSummaryHandoffContractsTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.CampaignPlanner.TestSupport
  alias OrbitalDynamics.Schema

  @repair_source "campaign_repair.source_provider_counteroffer_review_summary.review_rows"
  ...
  test "validates Repair source provider-counteroffer review-summary handoffs in producer order",
       %{repair: repair, source_summary: source_summary} do
    assert {:ok, %{"schema_version" => 2}} = Schema.validate_artifact(repair)

    expected_source_rows = enriched_review_rows(source_summary)
  ...
  test "keeps additive source provider-counteroffer review-summary handoffs optional", %{
    repair: repair
  } do
  ...
  test "rejects Repair source provider-counteroffer review-summary handoff drift", %{
    repair: repair
  } do
  ...
  defp source_summary do
    source_report()
    |> OrbitalDynamics.provider_counteroffer_review_summary(now_s: 360.0)
  end

  defp no_review_summary do
    source_report()
    |> mark_ineligible(0)
    |> mark_ineligible(2)
    |> OrbitalDynamics.provider_counteroffer_review_summary(now_s: 360.0)
  ...
  defp enriched_review_rows(summary) do
