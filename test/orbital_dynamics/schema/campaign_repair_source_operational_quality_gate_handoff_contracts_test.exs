defmodule OrbitalDynamics.Schema.CampaignRepairSourceOperationalQualityGateHandoffContractsTest do
  ...
  @source_base "campaign_repair.source_operational_quality_gate_summary"
  @source @source_base <> ".rows"
  ...
  setup_all do
    ...
    repair =
      ...
      |> Map.put("source_operational_quality_gate_summary", summary)
    ...
    expected_rows = QualityGate.source_report_rows(summary, @source_base)

    %{repair: repair, expected_rows: expected_rows}
  end

  test "replays the normalized operational quality-gate summary rows", %{
    repair: repair,
  ...
  test "rejects source operational quality-gate identity drift at every produced path", %{
    repair: repair
  } do
  ...
  test "rejects normalized row and report drift at every produced path", %{repair: repair} do
    review_index = source_review_index(repair)
    import_index = source_import_index(repair)

    invalid_cases = [
      {"$.operator_review_package.rows[#{review_index}].source_quality_gate_row",
       put_quality_gate_rank(repair, "operator_review_package", review_index, false)},
      {"$.cadence_import_manifest.rows[#{import_index}].source_quality_gate_row",
       put_quality_gate_rank(repair, "cadence_import_manifest", import_index, false)},
      {"$.cadence_import_manifest.rows[#{import_index}].source_review_row.source_quality_gate_row",
       put_quality_gate_rank(repair, "cadence_import_manifest", import_index, true)},
  ...
  test "rejects coordinated report drift, missing rows, and stale handoffs", %{repair: repair} do
    ...
    stale_handoffs = Map.delete(repair, "source_operational_quality_gate_summary")
  ...
  defp put_quality_gate_rank(repair, package, index, nested?) do
    put_in(
      repair,
      [package, "rows", Access.at(index)] ++ quality_gate_rank_path(nested?),
      99
    )
  end
  ...
  defp quality_gate_rank_path(true),
    do: ["source_review_row", "source_quality_gate_row", "rank"]

  defp quality_gate_rank_path(false), do: ["source_quality_gate_row", "rank"]
