...
defmodule OrbitalDynamics.Schema.CampaignRepairSourceContactAllocationSummaryHandoffContractsTest do
  ...
  @plural_prefix "campaign_repair.source_contact_allocation_summaries[0]"
  @plural_source @plural_prefix <> ".review_rows"
  @singular_source "campaign_repair.source_contact_allocation_summary.review_rows"
  ...
  test "validates ordered Repair source compact allocation-summary review handoffs", %{
    repair: repair,
    source_summary: source_summary
  } do
    ...
    assert Enum.map(review_rows, &summary_schema_contract/1) ==
             List.duplicate("contact_allocation_summary.v1", length(source_contact_ids))
  ...
  test "validates the singular compact allocation-summary compatibility fallback", %{
  ...
  test "keeps additive source compact allocation-summary handoffs optional", %{
    repair: repair
  } do
  ...
  test "rejects Repair source compact allocation-summary handoff drift", %{
    repair: repair
  } do
    ...
    empty_summary =
      ...
      |> OrbitalDynamics.contact_allocation_summary()

    eligibility_drift =
      repair
      |> Map.put("source_contact_allocation_summary", empty_summary)
      |> Map.put("source_contact_allocation_summaries", [empty_summary])
