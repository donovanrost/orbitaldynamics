defmodule OrbitalDynamics.Schema.CampaignRepairContactIntentSummarySourceContractsTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.Schema

  @source_field "source_contact_intent_summary"
  ...
  test "validates the optional V2 source contact-intent summary", %{artifact: artifact} do
  ...
  test "rejects source contact-intent drift at the distinct V2 path", %{artifact: artifact} do
    invalid_count = put_in(artifact, [@source_field, "contact_intent_count"], 99)

    assert {:error, count_report} = Schema.validate_artifact(invalid_count)

    assert Enum.any?(
             count_report["errors"],
             &(&1["path"] == "$.#{@source_field}.contact_intent_count")
           )
  ...
  test "exports the source contact-intent summary property" do
    assert {:ok, schema} = Schema.json_schema("campaign_repair.v2")
    assert get_in(schema, ["properties", @source_field, "type"]) == "object"

    assert get_in(schema, ["$defs", "contact_intent_summary.v1", "type"]) == "object"
  end
