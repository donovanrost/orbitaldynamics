defmodule OrbitalDynamics.Schema.CampaignRepairContactAllocationHandoffContractsTest do
  ...
  test "validates checked Repair contact-allocation review handoffs", context do
  ...
  test "keeps additive contact-allocation review handoffs optional", %{
    ...
    review_index = allocation_review_index(repair)
    cadence_index = allocation_import_index(repair)
  ...
  test "rejects Repair contact-allocation review handoff drift", %{
    readiness_repair: repair
  } do
    review_index = allocation_review_index(repair)
    cadence_index = allocation_import_index(repair)
    ...
    review_copy_drift =
      update_in(
        ...
        &Map.put(&1, "contact_status", "drifted")
      )
    ...
    invalid_cases = [
      ...
      {"$.operator_review_package.rows[#{review_index}].source_contact_allocation",
       review_copy_drift},
      {"$.cadence_import_manifest.rows[#{cadence_index}].source_contact_allocation",
       cadence_copy_drift},
      {"$.cadence_import_manifest.rows[#{cadence_index}].source_review_row.source_contact_allocation",
       cadence_copy_drift}
    ]
  ...
  defp allocation_review_index(repair) do
    Enum.find_index(
      get_in(repair, ["operator_review_package", "rows"]),
      &(Map.get(&1, "review_type") == "contact_allocation_review" and
          Map.get(&1, "source") == @repair_allocation_source)
    )
  end

  defp allocation_import_index(repair) do
    Enum.find_index(
      get_in(repair, ["cadence_import_manifest", "rows"]),
      &(Map.get(&1, "source_review_type") == "contact_allocation_review" and
          get_in(&1, ["source_review_row", "source"]) == @repair_allocation_source)
    )
  end
