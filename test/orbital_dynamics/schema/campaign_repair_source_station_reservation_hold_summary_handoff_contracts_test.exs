defmodule OrbitalDynamics.Schema.CampaignRepairSourceStationReservationHoldSummaryHandoffContractsTest do
  ...
  @source "campaign_repair.source_station_reservation_hold_summary"
  ...
  setup_all do
    ...
    repair =
      ...
      |> Map.put("source_station_reservation_hold_summary", source_summary)
  ...
  test "validates exact hold-review eligibility, order, identity, and summary evidence", %{
    repair: repair,
  ...
  test "rejects source hold-summary identity drift at every produced path", %{
    repair: repair
  } do
  ...
  test "rejects source hold-summary copy drift at every evidence path", %{
    repair: repair
  } do
    review_index = source_review_index(repair)
    import_index = source_import_index(repair)
    ...
    invalid_cases = [
      {"$.operator_review_package.rows[#{review_index}].source_station_reservation",
       put_in(
         repair,
         hold_count_path("operator_review_package", review_index, false),
         99
       )},
      {"$.cadence_import_manifest.rows[#{import_index}].source_station_reservation",
       put_in(
         repair,
         hold_count_path("cadence_import_manifest", import_index, false),
         99
       )},
      {"$.cadence_import_manifest.rows[#{import_index}].source_review_row.source_station_reservation",
       put_in(
         repair,
         hold_count_path("cadence_import_manifest", import_index, true),
         99
       )}
    ]
  ...
  test "rejects coordinated hold-count drift, missing rows, and stale handoffs", %{
    repair: repair
  } do
    review_index = source_review_index(repair)
    import_index = source_import_index(repair)
    ...
    coordinated_drift =
      ...
      |> update_in(
        ...
        &put_hold_count_drift(&1, false)
      )
      |> update_in(
        ["cadence_import_manifest", "rows", Access.at(import_index)],
        &put_hold_count_drift(&1, true)
      )
    ...
    stale_handoffs = Map.delete(repair, "source_station_reservation_hold_summary")
  ...
  defp put_hold_count_drift(row, nested?) do
    row =
      put_in(
        row,
        [
  ...
  defp hold_count_path(package, index, nested?) do
    ...
    prefix ++ evidence_path ++ ["source_station_reservation_summary", "reservation_hold_count"]
  end
