defmodule OrbitalDynamics.Schema.CampaignRepairTimelineDiffSummarySourceContractsTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.Schema

  @source_field "source_timeline_diff_summary"
  ...
  test "validates the optional V2 source timeline-diff summary", %{artifact: artifact} do
  ...
  test "rejects source timeline-diff drift at the distinct V2 path", %{artifact: artifact} do
  ...
  test "exports the source timeline-diff summary property" do
    assert {:ok, schema} = Schema.json_schema("campaign_repair.v2")
    assert get_in(schema, ["properties", @source_field, "type"]) == "object"

    assert get_in(schema, ["$defs", "timeline_diff_summary.v1", "type"]) == "object"
  end
