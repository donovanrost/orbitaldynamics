defmodule OrbitalDynamics.Schema.CampaignRepairGeneratedContactAllocationCapacityPackHandoffContractsTest do
  ...
  setup_all do
    generated_report =
      ...
      |> read_json!()
      |> Map.put("source", "campaign_repair.activities")

    repair =
      "study_results/leo_constellation_campaign_repair_v2.json"
      |> read_json!()
      |> Map.drop(["operator_review_package", "cadence_import_manifest"])
      |> Map.put("contact_allocation_report", generated_report)
    ...
    expected_rows =
      ContactAllocation.capacity_pack_rows(
        generated_report["reduced_capacity_pack_groups"],
        @source
      )
  ...
  test "validates every generated reduced-capacity group in report order", %{
    repair: repair,
    expected_rows: expected_rows
  } do
    assert {:ok, %{"schema_version" => 2}} = Schema.validate_artifact(repair)

    assert [%{"source" => @source} = review_row] = generated_review_rows(repair)
    assert [import_row] = generated_import_rows(repair)
    ...
    older =
      ...
      |> update_in(["cadence_import_manifest", "rows"], fn rows ->
        Enum.map(rows, fn row ->
          if generated_capacity_pack_row?(row) do
            row
            |> drop_source_copy()
  ...
  test "rejects generated capacity-pack identity drift at every produced path", %{
    repair: repair
  } do
    review_index = generated_review_index(repair)
    import_index = generated_import_index(repair)
  ...
  test "rejects generated capacity-pack copy drift at every evidence path", %{
    repair: repair
  } do
    review_index = generated_review_index(repair)
    import_index = generated_import_index(repair)
  ...
  test "rejects coordinated pack-status drift, missing rows, and stale handoffs", %{
    ...
    stale_handoffs = Map.delete(repair, "contact_allocation_report")
  ...
  defp drop_source_copy(row) do
    if generated_capacity_pack_row?(row),
      do: Map.delete(row, "source_contact_allocation_capacity_pack"),
      else: row
  end

  defp generated_review_rows(repair) do
    Enum.filter(
      get_in(repair, ["operator_review_package", "rows"]),
      &generated_capacity_pack_row?/1
    )
  end

  defp generated_import_rows(repair) do
    Enum.filter(
  ...
  defp generated_review_index(repair) do
    Enum.find_index(
      get_in(repair, ["operator_review_package", "rows"]),
      &generated_capacity_pack_row?/1
    )
  ...
  defp generated_import_index(repair) do
    Enum.find_index(
      get_in(repair, ["cadence_import_manifest", "rows"]),
      &generated_capacity_pack_row?/1
    )
  ...
  defp generated_capacity_pack_row?(row) do
