defmodule OrbitalDynamics.Schema.CampaignRepairTimelineDependencyImpactSourceContractsTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.Schema

  @source_field "source_timeline_dependency_impact_summary"
  ...
  test "rejects source dependency-impact drift at the distinct V2 path", %{
    artifact: artifact
  } do
    invalid_count = put_in(artifact, [@source_field, "changed_source_activity_count"], 99)

    assert {:error, count_report} = Schema.validate_artifact(invalid_count)

    assert Enum.any?(
             count_report["errors"],
             &(&1["path"] == "$.#{@source_field}.changed_source_activity_count")
           )
  ...
  test "exports the source dependency-impact property" do
    assert {:ok, schema} = Schema.json_schema("campaign_repair.v2")
    assert get_in(schema, ["properties", @source_field, "type"]) == "object"

    assert get_in(schema, ["$defs", "timeline_dependency_impact_summary.v1", "type"]) ==
             "object"
  end
