defmodule OrbitalDynamics.Schema.CampaignRepairContactAllocationProviderReservationRequestSummariesSourceContractsTest do
  ...
  test "validates every ordered V2 source provider-reservation-request summary", %{
    artifact: artifact
  } do
  ...
  test "rejects indexed source drift and a non-list plural shape", %{artifact: artifact} do
    invalid_count =
      put_in(
        artifact,
        [@plural_field, Access.at(1), "provider_reservation_request_contact_count"],
        99
      )
    ...
    assert Enum.any?(
             count_report["errors"],
             &(&1["path"] ==
                 "$.#{@plural_field}[1].provider_reservation_request_contact_count")
           )
  ...
  test "exports the plural source provider-reservation-request property" do
