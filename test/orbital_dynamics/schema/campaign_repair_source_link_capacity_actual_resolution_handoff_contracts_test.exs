...
defmodule OrbitalDynamics.Schema.CampaignRepairSourceLinkCapacityActualResolutionHandoffContractsTest do
  ...
  test "validates Repair source link-capacity actual-resolution handoffs", %{
    repair: repair,
    source_report: source_report
  } do
  ...
  test "keeps additive source link-capacity actual-resolution handoffs optional", %{
    repair: repair
  } do
  ...
  test "rejects Repair source link-capacity actual-resolution handoff drift", %{
    repair: repair
  } do
    invalid_cases =
  ...
  defp source_report do
    OrbitalDynamics.link_capacity_report(
      ...
      policy: %{required_downlink_mb: 50.0}
    )
  end
