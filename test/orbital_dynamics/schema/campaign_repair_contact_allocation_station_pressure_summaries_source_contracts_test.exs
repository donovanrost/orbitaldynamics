defmodule OrbitalDynamics.Schema.CampaignRepairContactAllocationStationPressureSummariesSourceContractsTest do
  ...
  test "validates every ordered V2 source station-pressure summary", %{artifact: artifact} do
    assert {:ok, %{"schema_contract" => "campaign_repair.v2"}} =
             Schema.validate_artifact(artifact)

    assert Enum.all?(artifact[@plural_field], fn summary ->
             match?(
               {:ok, %{"schema_contract" => "contact_allocation_station_pressure_summary.v1"}},
  ...
  test "rejects indexed source drift and a non-list plural shape", %{artifact: artifact} do
    invalid_count =
      put_in(artifact, [@plural_field, Access.at(1), "station_pressure_contact_count"], 99)

    assert {:error, count_report} = Schema.validate_artifact(invalid_count)

    assert Enum.any?(
             count_report["errors"],
             &(&1["path"] == "$.#{@plural_field}[1].station_pressure_contact_count")
           )
  ...
  test "exports the plural source station-pressure property" do
    assert {:ok, schema} = Schema.json_schema("campaign_repair.v2")
    assert get_in(schema, ["properties", @plural_field, "type"]) == "array"

    assert get_in(
             schema,
             ["$defs", "contact_allocation_station_pressure_summary.v1", "type"]
           ) == "object"
  end
