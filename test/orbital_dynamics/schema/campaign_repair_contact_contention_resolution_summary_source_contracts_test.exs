defmodule OrbitalDynamics.Schema.CampaignRepairContactContentionResolutionSummarySourceContractsTest do
  use ExUnit.Case, async: true

  alias OrbitalDynamics.Schema

  @source_field "source_contact_contention_resolution_summary"
  ...
  setup do
    ...
    source_summary = read_json!("study_results/contact_contention_resolution_summary_v1.json")

    %{artifact: Map.put(artifact, @source_field, source_summary)}
  end

  test "validates the optional V2 source compact resolution summary", %{artifact: artifact} do
  ...
  test "rejects source summary drift at the distinct V2 path", %{artifact: artifact} do
    invalid_count = put_in(artifact, [@source_field, "recommendation_count"], 99)

    assert {:error, count_report} = Schema.validate_artifact(invalid_count)

    assert Enum.any?(
             count_report["errors"],
             &(&1["path"] == "$.#{@source_field}.recommendation_count")
           )
  ...
  test "exports the source compact resolution-summary property" do
    assert {:ok, schema} = Schema.json_schema("campaign_repair.v2")
    assert get_in(schema, ["properties", @source_field, "type"]) == "object"

    assert get_in(schema, ["$defs", "contact_contention_resolution_summary.v1", "type"]) ==
             "object"
  end
