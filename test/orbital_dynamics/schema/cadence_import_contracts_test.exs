defmodule OrbitalDynamics.Schema.CadenceImportContractsTest do
  ...
  test "validates Cadence import source-window handoff rows" do
    ...
    assert {:ok, _manifest} = Schema.validate_artifact(routed_provider_review_identity)
    ...
    assert {:ok, _manifest} = Schema.validate_artifact(valid_provider_review_identity)
    ...
    invalid_provider_review_route =
      Map.put(manifest, "provider_reservation_review_contact_ids_by_direction", %{
    ...
    assert {:ok, _manifest} = Schema.validate_artifact(routed_provider_request_identity)
    ...
    assert {:ok, _manifest} = Schema.validate_artifact(valid_provider_request_identity)
    ...
    invalid_provider_request_route =
      Map.put(manifest, "provider_reservation_request_contact_ids_by_direction", %{
        "downlink" => ["contact_b", "contact_a"]
      })
    ...
    assert {:ok, _manifest} = Schema.validate_artifact(routed_provider_no_request_identity)
    ...
    assert {:ok, _manifest} = Schema.validate_artifact(valid_provider_no_request_identity)
    ...
    invalid_provider_no_request_route =
      Map.put(manifest, provider_no_request_direction_field, %{
        "downlink" => ["contact_b", "contact_a"]
      })
  ...
  test "correlates station-reservation contact IDs with match-status counts" do
    ...
    count_only = Map.put(manifest, "station_reservation_match_status_counts", %{status => 2})
    assert {:ok, _manifest} = Schema.validate_artifact(count_only)
  ...
  test "correlates top-level and routed station-reservation identity" do
    ...
    route_only =
      Map.put(manifest, "station_reservation_ids_by_match_status", %{
        "overlap" => ["reservation_routed"]
      })

    assert {:ok, _manifest} = Schema.validate_artifact(route_only)
  ...
  test "correlates station-reservation owner and status vocabularies" do
    ...
    assert {:ok, _manifest} = Schema.validate_artifact(valid_vocabularies)

    route_only =
      Map.put(manifest, "station_reservation_ids_by_reserved_by", %{
        "owner_route" => ["reservation_owner"]
      })
  ...
  test "correlates station-reservation contact IDs with owner counts" do
    ...
    count_only = Map.put(manifest, "station_reserved_by_counts", %{owner => 2})
    assert {:ok, _manifest} = Schema.validate_artifact(count_only)
  ...
  test "correlates required-capacity contact IDs with source counts" do
    ...
    count_only = Map.put(manifest, "required_capacity_fraction_source_counts", %{source => 2})
    assert {:ok, _manifest} = Schema.validate_artifact(count_only)
  ...
  test "correlates provider-reservation status observations with contact evidence" do
    ...
    for {status, contact_count_field} <- [
      ...
      assert {:ok, _manifest} = Schema.validate_artifact(legacy_status_only)
  ...
  test "preserves partial branch windows on strategy source-review handoffs" do
    ...
    tradeoff_row =
      ...
      |> Map.merge(%{
        ...
        "branch_untimed_source_window_count" => 1,
        "branch_partially_timed_source_window_ids" => ["window_d"],
