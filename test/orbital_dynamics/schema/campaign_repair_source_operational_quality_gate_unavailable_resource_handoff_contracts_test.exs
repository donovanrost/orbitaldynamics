defmodule OrbitalDynamics.Schema.CampaignRepairSourceOperationalQualityGateUnavailableResourceHandoffContractsTest do
  ...
  setup_all do
    ...
    repair =
      ...
      |> Map.put("source_operational_quality_gate_unavailable_resource_summary", summary)
  ...
  test "replays the normalized unavailable-resource quality-gate row", %{
    repair: repair,
  ...
  test "rejects unavailable-resource source identity drift at every produced path", %{
    repair: repair
  } do
  ...
  test "rejects normalized resource row and report drift at every produced path", %{
    repair: repair
  } do
    review_index = source_review_index(repair)
    import_index = source_import_index(repair)
    ...
    invalid_cases = [
      {"$.operator_review_package.rows[#{review_index}].source_quality_gate_row",
       put_resource_pressure(repair, "operator_review_package", review_index, false)},
      {"$.cadence_import_manifest.rows[#{import_index}].source_quality_gate_row",
       put_resource_pressure(repair, "cadence_import_manifest", import_index, false)},
      {"$.cadence_import_manifest.rows[#{import_index}].source_review_row.source_quality_gate_row",
       put_resource_pressure(repair, "cadence_import_manifest", import_index, true)},
  ...
  test "rejects coordinated report drift, missing rows, and stale handoffs", %{repair: repair} do
    ...
    stale_handoffs =
      Map.delete(repair, "source_operational_quality_gate_unavailable_resource_summary")
  ...
  defp put_resource_pressure(repair, package, index, nested?) do
    put_in(
      repair,
      [package, "rows", Access.at(index)] ++ resource_pressure_path(nested?),
      99
    )
  end
  ...
  defp resource_pressure_path(true),
    do: ["source_review_row", "source_quality_gate_row", "resource_availability_pressure_count"]

  defp resource_pressure_path(false),
    do: ["source_quality_gate_row", "resource_availability_pressure_count"]
