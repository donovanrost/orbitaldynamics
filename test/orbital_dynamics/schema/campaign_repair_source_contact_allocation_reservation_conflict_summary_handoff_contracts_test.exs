...
defmodule OrbitalDynamics.Schema.CampaignRepairSourceContactAllocationReservationConflictSummaryHandoffContractsTest do
  ...
  @plural_prefix "campaign_repair.source_contact_allocation_reservation_conflict_summaries[0]"
  @plural_source @plural_prefix <> ".reservation_review_rows"
  @singular_source "campaign_repair.source_contact_allocation_reservation_conflict_summary.reservation_review_rows"
  ...
  test "validates ordered Repair source reservation-conflict-summary review handoffs", %{
    repair: repair,
    source_summary: source_summary
  } do
    ...
    source_contact_ids = Enum.map(source_summary["reservation_review_rows"], & &1["contact_id"])

    assert source_contact_ids == ["dl_reserved_intruder"]

    assert Enum.map(source_summary["rows"], & &1["contact_id"]) == [
             "dl_reserved_owner",
  ...
  test "validates the singular reservation-conflict-summary compatibility fallback", %{
  ...
  test "keeps additive source reservation-conflict-summary handoffs optional", %{
    repair: repair
  } do
  ...
  test "rejects Repair source reservation-conflict-summary handoff drift", %{
    repair: repair
  } do
    ...
    empty_summary =
      ...
      |> OrbitalDynamics.contact_allocation_reservation_conflict_summary()

    eligibility_drift =
      repair
      |> Map.put("source_contact_allocation_reservation_conflict_summary", empty_summary)
      |> Map.put("source_contact_allocation_reservation_conflict_summaries", [empty_summary])
