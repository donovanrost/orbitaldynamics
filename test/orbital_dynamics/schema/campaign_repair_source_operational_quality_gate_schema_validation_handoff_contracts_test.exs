defmodule OrbitalDynamics.Schema.CampaignRepairSourceOperationalQualityGateSchemaValidationHandoffContractsTest do
  ...
  setup_all do
    ...
    repair =
      ...
      |> Map.put("source_operational_quality_gate_schema_validation_summary", summary)
  ...
  test "replays the normalized schema-validation quality-gate row", %{
    repair: repair,
  ...
  test "rejects schema-validation source identity drift at every produced path", %{
    repair: repair
  } do
  ...
  test "rejects normalized validation row and report drift at every produced path", %{
    repair: repair
  } do
    review_index = source_review_index(repair)
    import_index = source_import_index(repair)
    ...
    invalid_cases = [
      {"$.operator_review_package.rows[#{review_index}].source_quality_gate_row",
       put_schema_fail_count(repair, "operator_review_package", review_index, false)},
      {"$.cadence_import_manifest.rows[#{import_index}].source_quality_gate_row",
       put_schema_fail_count(repair, "cadence_import_manifest", import_index, false)},
      {"$.cadence_import_manifest.rows[#{import_index}].source_review_row.source_quality_gate_row",
       put_schema_fail_count(repair, "cadence_import_manifest", import_index, true)},
  ...
  test "rejects coordinated report drift, missing rows, and stale handoffs", %{repair: repair} do
    ...
    stale_handoffs =
      Map.delete(repair, "source_operational_quality_gate_schema_validation_summary")
  ...
  defp put_schema_fail_count(repair, package, index, nested?) do
    put_in(
      repair,
      [package, "rows", Access.at(index)] ++ schema_fail_count_path(nested?),
      99
    )
  end
  ...
  defp schema_fail_count_path(true),
    do: ["source_review_row", "source_quality_gate_row", "schema_validation_fail_count"]

  defp schema_fail_count_path(false),
    do: ["source_quality_gate_row", "schema_validation_fail_count"]
  ...
  defp source_quality_gate_row?(row), do: row_source(row) == @source
