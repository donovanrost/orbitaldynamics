defmodule OrbitalDynamics.Schema.CampaignRepairContactAllocationCapacityPackSummariesSourceContractsTest do
  ...
  test "validates every ordered V2 source capacity-pack summary", %{artifact: artifact} do
    assert {:ok, %{"schema_contract" => "campaign_repair.v2"}} =
             Schema.validate_artifact(artifact)

    assert Enum.all?(artifact[@plural_field], fn summary ->
             match?(
               {:ok, %{"schema_contract" => "contact_allocation_capacity_pack_summary.v1"}},
  ...
  test "rejects drift in the singular compatibility mirror", %{artifact: artifact} do
    drifted =
      update_in(artifact, [@singular_field], &Map.put(&1, "compatibility_marker", "drift"))

    assert {:error, report} = Schema.validate_artifact(drifted)
  ...
  test "exports the plural source capacity-pack property" do
