defmodule OrbitalDynamics.CandidateRefresh.ReplaySummary.ContactFilter.SourceReportFields.Aggregation do
  @moduledoc false

  alias OrbitalDynamics.CandidateRefresh.ValueEncoding

  def source_report_family_count(source_reports, field) do
    if Map.has_key?(source_reports, "contact_filter_report") do
      source_reports
  ...
  defp source_report_family_reports(source_reports) do
    source_reports
    |> Map.take(["contact_filter_report"])
    |> Map.values()
  end

  defp source_report_family_has_identity_counts?(source_reports) do
    case Map.get(source_reports, "contact_filter_report") do
      %{} = summary ->
