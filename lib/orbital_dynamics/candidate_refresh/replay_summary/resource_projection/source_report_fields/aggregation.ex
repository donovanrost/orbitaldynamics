defmodule OrbitalDynamics.CandidateRefresh.ReplaySummary.ResourceProjection.SourceReportFields.Aggregation do
  @moduledoc false

  alias __MODULE__.Values

  def source_report_family_count(source_reports, field) do
    if Map.has_key?(source_reports, "resource_projection_report") do
      source_reports
  ...
  defp source_report_family_reports(source_reports) do
    source_reports
    |> Map.take(["resource_projection_report"])
    |> Map.values()
  end

  defp source_report_family_has_identity_counts?(source_reports) do
    case Map.get(source_reports, "resource_projection_report") do
      %{} = summary ->
