defmodule OrbitalDynamics.CandidateRefresh.ReplaySummary.Candidate.Rejection.SourceReportFields.Detail do
  @moduledoc false
  ...
  def fields(source_reports) do
    %{
      ...
      "source_report_candidate_rejection_rejected_count" =>
        source_report_family_count(source_reports, "rejected_count"),
      "source_report_candidate_rejection_reviewable_count" =>
        source_report_family_count(source_reports, "reviewable_count"),
      "source_report_candidate_rejection_invalid_candidate_input_count" =>
        source_report_family_count(source_reports, "invalid_candidate_input_count"),
  ...
  defp source_report_family_reports(source_reports) do
    source_reports
    |> Map.take(["candidate_rejection_report"])
    |> Map.values()
  end

  defp source_report_family_count(source_reports, field) do
    if Map.has_key?(source_reports, "candidate_rejection_report") do
      source_reports
  ...
  defp source_report_family_has_identity_counts?(source_reports) do
    case Map.get(source_reports, "candidate_rejection_report") do
      %{} = summary ->
