defmodule OrbitalDynamics.CandidateRefresh.ReplaySummary.LinkCapacity.SourceReportFields.Aggregation do
  @moduledoc false
  ...
  def source_report_family_reports(source_reports) do
    source_reports
    |> Map.take(["link_capacity_report"])
    |> Map.values()
  end

  def source_report_family_count(source_reports, field) do
    if Map.has_key?(source_reports, "link_capacity_report") do
      source_reports
  ...
  def source_report_family_has_identity_counts?(source_reports) do
    case Map.get(source_reports, "link_capacity_report") do
      %{} = summary ->
