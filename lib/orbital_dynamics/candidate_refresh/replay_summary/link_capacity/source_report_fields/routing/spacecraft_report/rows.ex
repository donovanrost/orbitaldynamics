defmodule OrbitalDynamics.CandidateRefresh.ReplaySummary.LinkCapacity.SourceReportFields.Routing.SpacecraftReport.Rows do
  @moduledoc false

  alias __MODULE__.SpacecraftPairs
  alias __MODULE__.SummaryValues
  alias __MODULE__.SummaryValues.GroupedIds

  def spacecraft_contact_pairs(row) do
    SpacecraftPairs.spacecraft_contact_pairs(row)
  end

  def spacecraft_source_window_pairs(row) do
    SpacecraftPairs.spacecraft_source_window_pairs(row)
  end

  def spacecraft_station_calendar_entry_pairs(row) do
    SpacecraftPairs.spacecraft_station_calendar_entry_pairs(row)
  end

  def spacecraft_station_calendar_provider_entry_pairs(row) do
    SpacecraftPairs.spacecraft_station_calendar_provider_entry_pairs(row)
  end
