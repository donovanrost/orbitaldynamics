defmodule OrbitalDynamics.CandidateRefresh.ReplaySummary.OperationalReadiness.SourceReportFields.Flattened.Aggregates do
  @moduledoc false

  def count(source_reports, field) do
    if Map.has_key?(source_reports, "operational_readiness_report") do
      source_reports
  ...
  def field(source_reports, field) do
    source_reports
    |> Map.get("operational_readiness_report", %{})
    |> Map.get(field)
  end
  ...
  defp source_report_family_reports(source_reports) do
    source_reports
    |> Map.take(["operational_readiness_report"])
    |> Map.values()
  end

  defp source_report_family_has_identity_counts?(source_reports) do
    case Map.get(source_reports, "operational_readiness_report") do
      %{} = summary ->
