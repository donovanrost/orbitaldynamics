defmodule OrbitalDynamics.CandidateRefresh.SourceReports.LinkConstraintCollectionConstraintReviewArtifactDirectReports do
  @moduledoc false

  def reports(refresh, entries, source_key, artifact_key) do
    refresh
    |> sources(source_key, artifact_key)
    |> Enum.flat_map(fn {path, artifact_or_artifacts} ->
      entries.(path, artifact_or_artifacts)
    end)
  end

  defp sources(refresh, source_key, artifact_key) do
    [
