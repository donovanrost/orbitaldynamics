defmodule OrbitalDynamics.OperatorReview.ContactAllocationSummary do
  ...
  def put_repair(package, artifact) do
    artifact = stringify_keys(artifact || %{})
    {summary_reports, _source} = repair_summary_source(artifact)
    {station_pressure_reports, _source} = repair_station_pressure_source(artifact)
  ...
  def repair_summary_source(artifact) do
    artifact = stringify_keys(artifact || %{})

    case artifact["source_contact_allocation_summaries"] do
      [_summary | _summaries] = summaries ->
        {summaries, "campaign_repair.source_contact_allocation_summaries"}
  ...
  def repair_reservation_conflict_source(artifact) do
    ...
    case artifact["source_contact_allocation_reservation_conflict_summaries"] do
      [_summary | _summaries] = summaries ->
        {summaries, "campaign_repair.source_contact_allocation_reservation_conflict_summaries"}
  ...
  def repair_station_pressure_source(artifact) do
    artifact = stringify_keys(artifact || %{})

    case artifact["source_contact_allocation_station_pressure_summaries"] do
      [_summary | _summaries] = summaries ->
        {summaries, "campaign_repair.source_contact_allocation_station_pressure_summaries"}
  ...
  defp put_station_reservation_identity_summary(package, reports) do
    direct_identity_lists =
      Enum.flat_map(reports, fn report ->
        collect_identity_lists(report["station_reservation_ids"])
      end)

    routed_id_maps =
      Map.new(@station_reservation_id_map_fields, fn field ->
        id_map =
          reports
          |> Enum.map(&Map.get(&1, field))
          |> Enum.filter(&is_map/1)
    ...
    package =
      Enum.reduce(routed_id_maps, package, fn {field, id_map}, acc ->
        put_merged_id_map(acc, field, id_map)
      end)

    routed_identity_lists =
      Enum.flat_map(routed_id_maps, fn {_field, id_map} -> collect_identity_lists(id_map) end)

    case direct_identity_lists ++ routed_identity_lists do
      [] ->
        package
