defmodule OrbitalDynamics.OperatorReview.TimelineLifecycleState do
  ...
  defp source_activity_state_rows(states, source) when is_list(states) do
    states
    |> Enum.with_index()
  ...
  defp source_activity_state_rows(%{} = state, source) do
    state
    |> stringify_keys()
    |> activity_state_rows("#{source}.state")
  end

  defp source_activity_state_rows(_state, _source), do: []
