defmodule OrbitalDynamics.Schema.CadenceReviewSchemaProviders do
  ...
  def build(stable_id_pattern, opts) when is_binary(stable_id_pattern) and is_list(opts) do
    ...
    %{
      {:cadence_import_manifest_row_json_schema, 0} => fn ->
        OrbitalDynamics.Schema.CadenceImportManifestJsonSchema.row(
          capability: call(dependencies, :cadence_import_capability),
          readiness_capability: call(dependencies, :readiness_capability),
          stable_id_pattern: stable_id_pattern,
          schema_providers: manifest_schema_providers(stable_id_pattern, dependencies),
  ...
  defp source_review_row(stable_id_pattern, dependencies) do
    OrbitalDynamics.Schema.CadenceSourceReviewRowJsonSchema.row(
      cadence_capability: call(dependencies, :cadence_import_capability),
      readiness_capability: call(dependencies, :readiness_capability),
      timeline_capability: call(dependencies, :timeline_capability),
      stable_id_pattern: stable_id_pattern,
      schema_providers: source_schema_providers(stable_id_pattern, dependencies),
      property_providers: property_providers(stable_id_pattern, Map.to_list(dependencies))
    )
  end

  defp manifest_schema_providers(stable_id_pattern, dependencies) do
    source_schema_providers(stable_id_pattern, dependencies)
    |> List.insert_at(
      3,
    ...
    |> List.insert_at(
      ...
      {:cadence_source_review_row_json_schema,
       fn -> source_review_row(stable_id_pattern, dependencies) end}
    )
    |> List.insert_at(10, {:number_array_schema, &CommonJsonSchema.number_array/0})
    |> List.insert_at(
      11,
  ...
  defp source_schema_providers(stable_id_pattern, dependencies) do
    [
      ...
      candidate_rejection_source_json_schema:
        dependency(dependencies, :candidate_rejection_source_schema),
      ...
      source_execution_report_evidence_json_schema:
        dependency(dependencies, :source_execution_report_evidence_schema),
      source_freshness_report_evidence_json_schema:
        dependency(dependencies, :source_freshness_report_evidence_schema),
      source_schema_validation_report_evidence_json_schema:
        dependency(dependencies, :source_schema_validation_report_evidence_schema),
