defmodule OrbitalDynamics.Schema.TimelineContextValidation do
  ...
  def validate_optional_timeline_preconditions(issues, path, map, field) when is_map(map) do
  ...
  def validate_optional_activity_context(issues, path, map, field) when is_map(map) do
  ...
  def validate_optional_protection_decision(issues, path, map, field) when is_map(map) do
  ...
  def validate_optional_lifecycle_transition(issues, path, map, field) when is_map(map) do
  ...
  def validate_optional_timeline_identity(issues, path, map, field) when is_map(map) do
  ...
  def validate_optional_timeline_link(issues, path, map, field) when is_map(map) do
  ...
  def validate_optional_timeline_protection_summary(issues, path, map, field) when is_map(map) do
