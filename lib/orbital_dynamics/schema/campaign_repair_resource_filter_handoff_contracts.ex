defmodule OrbitalDynamics.Schema.CampaignRepairResourceFilterHandoffContracts do
  @moduledoc false
  ...
  defp validate_operator_handoff(
    ...
    |> validate_equal(
      ...
      "must contain one source resource-filter review row per suppressed candidate"
    )
    |> validate_source_identities(
      ...
      expected_sources,
      [["source"]],
      "must match the enclosing source resource-filter family and identity"
    )
    |> validate_source_copies(
      ...
      "must match the corresponding enclosing source resource-filter suppression"
    )
  end
  ...
  defp validate_cadence_handoff(
    ...
    |> validate_equal(
      ...
      length(import_rows),
      length(expected_sources),
      "must contain one source resource-filter import row per suppressed candidate"
    )
    |> validate_source_identities(
      "$.cadence_import_manifest.rows",
      import_rows,
  ...
  defp source_rows(%{"source_resource_filter_report" => %{} = report}) do
    candidates =
      report
    ...
    Suppression.resource_rows(candidates, @source)
  end
  ...
  defp operator_suppression_row?(row) do
    Map.get(row, "review_type") == "resource_suppression" and
      resource_filter_source?(row_source(row))
  end
  ...
  defp resource_filter_source?(source) when is_binary(source),
    do: String.starts_with?(source, @source)

  defp resource_filter_source?(_source), do: false
end
