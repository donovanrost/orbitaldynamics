defmodule OrbitalDynamics.Schema.CampaignRepairCommandWindowHandoffContracts do
  ...
  def validate(
        issues,
        %{"command_window_report" => %{"rows" => command_window_rows}} = artifact
      )
  ...
  defp validate_operator_review_handoff(
    ...
    review_rows = indexed_rows(Map.get(package, "rows"), &operator_command_window_row?/1)
    ...
    |> validate_equal(
      ...
      "must contain one generated Repair command-window review row per enclosing reviewable report row"
    )
    |> validate_source_copies(
      ...
      "must match the corresponding enclosing generated Repair command-window report row"
    )
  end

  defp validate_operator_review_handoff(issues, _artifact, _command_window_rows), do: issues
  ...
  defp validate_cadence_handoff(
    ...
    import_rows = indexed_rows(Map.get(manifest, "rows"), &cadence_command_window_row?/1)
    ...
    |> validate_equal(
      ...
      length(import_rows),
      length(command_window_rows),
      "must contain one generated Repair command-window import row per enclosing reviewable report row"
    )
    |> validate_source_copies(
      "$.cadence_import_manifest.rows",
      import_rows,
  ...
  defp validate_cadence_handoff(issues, _artifact, _command_window_rows), do: issues
  ...
  defp operator_command_window_row?(row) do
    Map.get(row, "review_type") == "command_window_review" and
      row_source(row) == @repair_command_window_source
  end

  defp cadence_command_window_row?(row) do
