defmodule OrbitalDynamics.Schema.TimelineFeedbackRowContracts do
  ...
  def validate(issues, path, row, callbacks) when is_list(callbacks) do
    require_fields = callback(callbacks, :require_fields)
    validate_stable_ids = callback(callbacks, :validate_stable_ids)
    expect_one_of = callback(callbacks, :expect_one_of)
    expect_optional_one_of = callback(callbacks, :expect_optional_one_of)
    expect_optional_type = callback(callbacks, :expect_optional_type)
    validate_optional_stable_id_list = callback(callbacks, :validate_optional_stable_id_list)
    validate_string_list_items = callback(callbacks, :validate_string_list_items)
    expect_optional_probability = callback(callbacks, :expect_optional_probability)
    expect_optional_integer = callback(callbacks, :expect_optional_integer)
    expect_field_at_least = callback(callbacks, :expect_field_at_least)
    validate_number_list_items = callback(callbacks, :validate_number_list_items)
    expect_optional_number = callback(callbacks, :expect_optional_number)
    validate_optional_activity_context = callback(callbacks, :validate_optional_activity_context)

    validate_optional_execution_uncertainty =
      callback(callbacks, :validate_optional_execution_uncertainty)

    validate_optional_protection_decision =
      callback(callbacks, :validate_optional_protection_decision)

    validate_optional_lifecycle_transition =
      callback(callbacks, :validate_optional_lifecycle_transition)

    validate_optional_actual_data_rate_throughput_derivation =
      callback(callbacks, :validate_optional_actual_data_rate_throughput_derivation)

    validate_resource_availability_variance_fields =
      callback(callbacks, :validate_resource_availability_variance_fields)

    validate_eclipse_lighting_handoff_fields =
      callback(callbacks, :validate_eclipse_lighting_handoff_fields)

    validate_link_handoff_fields = callback(callbacks, :validate_link_handoff_fields)

    validate_image_quality_score_fields =
      callback(callbacks, :validate_image_quality_score_fields)
    ...
    |> validate_optional_activity_context.(path, row, "source_activity_context")
    |> validate_optional_activity_context.(path, row, "realized_activity_context")
    ...
    |> validate_optional_execution_uncertainty.(path, row, "execution_uncertainty")
    ...
    |> validate_optional_protection_decision.(path, row, "source_protection_decision")
    |> expect_optional_type.(path, row, "status_transition", :map)
    |> validate_optional_lifecycle_transition.(path, row, "status_transition")
