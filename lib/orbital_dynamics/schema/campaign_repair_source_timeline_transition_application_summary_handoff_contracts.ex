defmodule OrbitalDynamics.Schema.CampaignRepairSourceTimelineTransitionApplicationSummaryHandoffContracts do
  @moduledoc false
  ...
  defp validate_operator_review_handoff(
    ...
    |> validate_equal(
      ...
      length(review_rows),
      length(source_rows),
      "must contain one Repair source timeline transition-application-summary review row per eligible enclosing review application"
    )
    |> validate_source_identities(
      ...
      expected_sources,
      [["source"]],
      "must match the enclosing Repair source timeline transition-application-summary source"
    )
    |> validate_source_copies(
      ...
      "must match the corresponding eligible enclosing Repair summary review application"
    )
    |> validate_source_copies(
      "$.operator_review_package.rows",
      review_rows,
      ...
      "must match the complete enclosing Repair source timeline transition-application summary"
    )
  end
  ...
  defp validate_cadence_handoff(
    ...
    |> validate_equal(
      ...
      length(import_rows),
      length(source_rows),
      "must contain one Repair source timeline transition-application-summary import row per eligible enclosing review application"
    )
    |> validate_source_identities(
      "$.cadence_import_manifest.rows",
      import_rows,
    ...
    |> validate_source_copies(
      ...
      [
        ["source_timeline_transition_application_summary"],
        ["source_review_row", "source_timeline_transition_application_summary"]
      ],
      "must match the complete enclosing Repair source timeline transition-application summary"
    )
  end
  ...
  defp review_required_applications(%{"review_applications" => applications})
  ...
  defp review_required_applications(_summary), do: []
end
