defmodule OrbitalDynamics.Schema.CampaignRepairConstraintContracts do
  @moduledoc false
  ...
  defp validate_report(issues, report) when is_map(report) do
    ...
    |> validate_equal(
      ...
      "must identify the V2 repair constraint model"
    )
    |> validate_equal(
      "$.constraint_report.assumptions.constraint_model",
      assumption(report, "constraint_model"),
      "campaign_repair_v2_planner_local_constraints",
      "must identify the V2 repair constraint assumptions"
    )
    |> validate_equal(
      "$.constraint_report.assumptions.source",
      assumption(report, "source"),
      "campaign_repair.assumptions.constraints",
      "must identify campaign_repair.assumptions.constraints"
    )
  end
