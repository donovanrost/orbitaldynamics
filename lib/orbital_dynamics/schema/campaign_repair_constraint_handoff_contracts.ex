defmodule OrbitalDynamics.Schema.CampaignRepairConstraintHandoffContracts do
  ...
  def validate(issues, artifact) when is_map(artifact) do
    constraint_rows = source_rows(artifact)
  ...
  defp source_rows(%{"source_constraint_report" => %{"rows" => rows}}) when is_list(rows),
    do: rows
  ...
  defp validate_cadence_handoff(
    ...
    |> validate_source_identities(
      "$.cadence_import_manifest.rows",
      import_rows,
      List.duplicate(@repair_source_constraint, length(constraint_rows)),
      [["source"], ["source_review_row", "source"]],
      "must match the enclosing Repair source constraint family"
    )
    |> validate_source_copies(
      "$.cadence_import_manifest.rows",
      import_rows,
