defmodule OrbitalDynamics.Schema.ContactAllocationValidation do
  ...
  def validate_optional_station_pressure_summaries(issues, _path, nil), do: issues

  def validate_optional_station_pressure_summaries(issues, path, summaries)
      when is_list(summaries) do
    summaries
    |> Enum.with_index()
    |> Enum.reduce(issues, fn
      {%{} = summary, index}, acc ->
        validate_station_pressure_artifact(acc, "#{path}[#{index}]", summary)
  ...
  def validate_optional_station_pressure_summaries(issues, path, _summaries),
  ...
  def validate_optional_provider_reservation_request_summaries(issues, _path, nil), do: issues

  def validate_optional_provider_reservation_request_summaries(issues, path, summaries)
      when is_list(summaries) do
    summaries
    |> Enum.with_index()
    |> Enum.reduce(issues, fn
      {%{} = summary, index}, acc ->
        validate_provider_reservation_request_artifact(acc, "#{path}[#{index}]", summary)
  ...
  def validate_optional_provider_reservation_request_summaries(issues, path, _summaries),
    do: [error(path, "must be a list") | issues]
