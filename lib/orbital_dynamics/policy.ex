defmodule OrbitalDynamics.Policy do
  ...
  defp approval_rule_requirement_match?(rule, requirement),
    do: RequirementMatcher.match?(rule, requirement)
  ...
  defp approval_rule_risk_match?(rule, risk), do: RiskMatcher.match?(rule, risk)

  defp risk_direction(risk), do: RiskMatcher.direction(risk)
  defp risk_direction_values(risk), do: RiskMatcher.directions(risk)
  defp risk_ground_station_id(risk), do: RiskMatcher.ground_station_id(risk)
  defp risk_spacecraft_id(risk), do: RiskMatcher.spacecraft_id(risk)
  defp risk_context_value(risk, field), do: RiskMatcher.context_value(risk, field)
  defp risk_context_values(risk, field), do: RiskMatcher.context_values(risk, field)

  defp approval_rule_event_match?(rule, event), do: EventMatcher.match?(rule, event)

  defp event_context_values(event, field), do: EventMatcher.context_values(event, field)
  defp event_direction(event), do: EventMatcher.direction(event)
  defp event_direction_values(event), do: EventMatcher.directions(event)
  defp event_ground_station_id(event), do: EventMatcher.ground_station_id(event)
  defp event_spacecraft_id(event), do: EventMatcher.spacecraft_id(event)

  defp approval_rule_feasibility_match?(rule, activity),
    do: ActivityMatcher.match?(rule, activity)

  defp activity_provenance_value(activity, field),
    do: ActivityMatcher.provenance_value(activity, field)

  defp activity_direction(activity), do: ActivityMatcher.direction(activity)
  defp activity_direction_values(activity), do: ActivityMatcher.directions(activity)
  defp activity_ground_station_id(activity), do: ActivityMatcher.ground_station_id(activity)
  defp activity_spacecraft_id(activity), do: ActivityMatcher.spacecraft_id(activity)
  defp activity_target_id(activity), do: ActivityMatcher.target_id(activity)
