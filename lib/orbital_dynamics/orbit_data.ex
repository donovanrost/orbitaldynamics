defmodule OrbitalDynamics.OrbitData do
  ...
  def inspect_tle(lines, opts) when is_list(lines) do
    with {:ok, object_name, line1, line2} <- tle_lines(lines),
         ...
         {:ok, metadata} <- tle_metadata(object_name, line1, line2, opts) do
      {:ok, metadata}
    end
  end
  ...
  defp tle_metadata(object_name, line1, line2, opts) do
  ...
  defp tle_mean_element_regime(mean_motion_rev_per_day, eccentricity)
    ...
    %{
      ...
      "altitude_regime" => tle_altitude_regime(perigee_altitude_km, apogee_altitude_km)
    }
  end

  defp tle_altitude_regime(perigee_altitude_km, apogee_altitude_km) do
    cond do
  ...
  defp build_accepted_planning_state([], _opts), do: {:error, {:missing_field, "state_estimates"}}

  defp build_accepted_planning_state([_ | _] = estimates, opts) do
  ...
  defp maneuver_execution_deltas(deltas, parent_provenance) when is_list(deltas) do
    deltas
    |> Enum.with_index()
  ...
  defp maneuver_execution_deltas(_deltas, _parent_provenance),
