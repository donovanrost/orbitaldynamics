defmodule OrbitalDynamics.EventDetectors.AccessWindows do
  ...
  def detect(%Trajectory{} = trajectory, opts \\ []) do
    ...
    with :ok <- validate_inputs(station, central_body) do
      samples =
        trajectory.states
        |> Enum.with_index()
      ...
      {:ok,
       samples
       |> visible_groups()
       |> Enum.map(&event_from_group(&1, samples, station, trajectory))
       |> Enum.map(&EventTiming.annotate_event(&1, trajectory, :access_windows))}
    end
  end
  ...
  The refinement is a linear interpolation over elevation margin relative to the
  station mask. It is useful for artifact metadata and coarse contact windows;
  it is not a root-solved event time.
  """
  ...
  defp event_from_group(group, samples, station, trajectory) do
    first = List.first(group)
    last = List.last(group)
    max_elevation_deg = group |> Enum.map(& &1.elevation_deg) |> Enum.max()
    {starts_at, start_boundary, start_boundary_detail} = start_boundary(first, samples, station)
    {ends_at, end_boundary, end_boundary_detail} = end_boundary(last, samples, station)
  ...
  defp start_boundary(first, samples, station) do
    if first.index == 0 do
      ...
      {interpolate_epoch(previous, first), :interpolated,
       boundary_detail(previous, first, station, :start)}
    end
  end

  defp end_boundary(last, samples, station) do
    if last.index == length(samples) - 1 do
      ...
      {interpolate_epoch(last, next_sample), :interpolated,
       boundary_detail(last, next_sample, station, :end)}
    end
  end
