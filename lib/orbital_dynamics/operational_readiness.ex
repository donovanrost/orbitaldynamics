defmodule OrbitalDynamics.OperationalReadiness do
  ...
  defp timeline_publication_context(artifact, review_rows, import_rows) do
    [artifact | review_rows ++ import_rows]
  ...
  defp timeline_publication_context_from_rows(rows) do
    contexts =
      rows
      |> Enum.filter(&is_map/1)
      |> Enum.map(
  ...
  defp timeline_publication_context_from_evidence(evidence) do
    evidence
