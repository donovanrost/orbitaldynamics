defmodule OrbitalDynamics.ResourceFilter do
  ...
  alias OrbitalDynamics.ResourceFilter.ApprovalPolicy
  alias OrbitalDynamics.ResourceFilter.CandidateInput
  ...
  def capabilities do
    %{
      ...
      provider_direction_aliases: CandidateInput.provider_direction_aliases(),
      station_calendar_direction_aliases: CandidateInput.station_calendar_direction_aliases(),
      provider_result_map_value_keys: @provider_result_map_value_keys,
      candidate_stable_identity_fields: CandidateInput.stable_identity_fields(),
      station_calendar_id_list_fields: CandidateInput.station_calendar_id_list_fields(),
      suppression_reasons: [
  ...
  def filter_candidates(candidates, summaries, opts)
    ...
    candidates =
      candidates
      |> Enum.with_index(1)
      |> Enum.map(fn {candidate, index} -> CandidateInput.normalize(candidate, index) end)
    ...
    invalid_summaries =
      Enum.map(
        invalid_summaries,
        &ApprovalPolicy.apply_invalid_summary(&1, approval_policy)
      )
    ...
    suppressed =
      ...
      |> Enum.map(&ApprovalPolicy.apply_suppressed_candidate(&1, approval_policy))
  ...
  defp suppressed_candidate(candidate, reason, summary) do
    %{
      ...
      "station_calendar_entry_id" => CandidateInput.station_calendar_entry_id(candidate),
    ...
    |> Map.merge(resource_summary_context(summary))
    |> CandidateInput.normalize_station_calendar_id_lists()
    |> compact_map()
  end
  ...
  defp stable_id?(value), do: CandidateInput.stable_id?(value)
  defp stable_id_or_nil(value), do: CandidateInput.stable_id_or_nil(value)
  ...
  defp normalize_resource_activity_token(value) when is_binary(value) do
    ...
    |> case do
      nil -> nil
      token -> Map.get(CandidateInput.resource_activity_type_aliases(), token, token)
    end
  end
