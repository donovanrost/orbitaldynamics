defmodule OrbitalDynamics.Communications.StationCalendar do
  ...
  alias OrbitalDynamics.Communications.StationCalendar.AffectedContact
  ...
  def overlay_contacts(contacts, station_calendar, opts)
      ...
      |> Enum.map_reduce([], fn contact, affected ->
        case station_calendar_matches(contact, entries) do
          ...
          matches ->
            ...
            {annotated, [AffectedContact.build(annotated, entry, matches) | affected]}
        end
      end)
    ...
    affected =
      affected
      |> Enum.reverse()
      |> AffectedContact.disambiguate_ids()
      |> Enum.map(&maybe_apply_approval_policy(&1, approval_policy))

    duplicate_affected_contact_id_groups = AffectedContact.duplicate_id_groups(affected)
    ...
    {annotated,
     %{
       ...
       "duplicate_affected_contact_row_count" =>
         AffectedContact.duplicate_row_count(duplicate_affected_contact_id_groups),
