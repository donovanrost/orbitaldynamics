defmodule OrbitalDynamics.Communications.ContactFilter do
  ...
  alias OrbitalDynamics.Communications.ContactFilter.StationState
  ...
  defp station_state(ground_network, ground_station_id, starts_at_s, ends_at_s, direction),
    do: StationState.resolve(ground_network, ground_station_id, starts_at_s, ends_at_s, direction)

  defp nonempty_station_directions(station), do: StationState.nonempty_directions(station)
  defp station_severity(station), do: StationState.severity(station)
  defp station_unavailable?(station), do: StationState.unavailable?(station)
  defp station_reserved?(station), do: StationState.reserved?(station)
  defp provider_counteroffer_review?(station), do: StationState.counteroffer_review?(station)

  defp station_reservation_matched?(candidate, station_state),
    do: StationState.reservation_matched?(candidate, station_state)
  ...
  defp station_capacity_fraction(station), do: StationState.capacity_fraction(station)
  defp station_availability(station), do: StationState.availability(station)
  defp reservation_expires_at_s(station), do: StationState.reservation_expires_at_s(station)
  defp station_contention_status(station), do: StationState.contention_status(station)
  ...
  defp station_reservation_match_status(candidate, station_state),
    do: StationState.reservation_match_status(candidate, station_state)
