defmodule OrbitalDynamics.CampaignPlanner.ContactContentionSourceReports do
  ...
  def contention_reports(mission_state),
    do: contention_reports(mission_state, default_callbacks())
  ...
  def resolution_reports(mission_state),
    do: resolution_reports(mission_state, default_callbacks())
  ...
  def prior_resolution_reports(prior_plan, opts \\ prior_plan_callbacks()) do
    ...
    SourceReportArtifacts.direct_reports(
      ...
      @prior_resolution_report_fields,
      &stringify_keys/1
    ) ++ prior_plan_result_artifact_resolution_reports(prior_plan, opts)
  end

  def candidate_refresh_source_inputs(mission_state) do
    report_inputs =
      (@contention_report_fields ++ @resolution_report_fields)
      |> Enum.map(&elem(&1, 0))
  ...
  def reports(mission_state, report_key) do
