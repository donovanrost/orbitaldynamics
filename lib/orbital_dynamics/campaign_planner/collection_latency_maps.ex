defmodule OrbitalDynamics.CampaignPlanner.CollectionLatencyMaps do
  @moduledoc false
  ...
  def encode_value(%_struct{} = struct), do: struct |> Map.from_struct() |> encode_value()

  def encode_value(%{} = map) do
    Map.new(map, fn {key, value} -> {encode_value(key), encode_value(value)} end)
  end

  def encode_value(values) when is_list(values), do: Enum.map(values, &encode_value/1)
