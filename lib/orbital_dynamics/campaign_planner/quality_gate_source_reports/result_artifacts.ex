defmodule OrbitalDynamics.CampaignPlanner.QualityGateSourceReports.ResultArtifacts do
  @moduledoc false
  ...
  def quality_gate_reports(prior_plan, opts) do
    callbacks = callbacks!(opts)

    prior_plan
    |> callbacks.result_artifacts_with_source.()
    |> Enum.flat_map(fn {artifact, source_path} ->
      quality_gate_reports(artifact, source_path, callbacks)
    end)
  end

  def operational_quality_gate_summary_reports(prior_plan, opts) do
    prior_plan
    |> stringify_keys()
    |> SourceReportArtifacts.embedded_reports(@operational_quality_gate_summary_report_keys, opts)
  end

  defp quality_gate_reports(artifact, source_path, callbacks) do
