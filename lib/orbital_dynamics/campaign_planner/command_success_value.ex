defmodule OrbitalDynamics.CampaignPlanner.CommandSuccessValue do
  @moduledoc false

  def value(%{} = activity, callbacks) do
    cond do
      Map.has_key?(activity, "command_success") ->
        command_success_field_value(activity, callbacks)

      Map.has_key?(activity, "command_result") ->
        command_result_field_value(activity, callbacks)
  ...
  defp command_success_field_value(activity, callbacks) do
    json_boolean_value = Keyword.fetch!(callbacks, :json_boolean_value)

    case json_boolean_value.(activity["command_success"]) do
      bool when is_boolean(bool) -> if(bool, do: 1.0, else: 0.0)
      nil -> activity |> Map.delete("command_success") |> value(callbacks)
    end
  end

  defp command_result_field_value(activity, callbacks) do
    ...
    case provider_result_success_value.(activity["command_result"]) do
      :failure -> 0.0
      :success -> completed_fraction_success_value.(activity, 1.0)
      :unknown -> activity |> Map.delete("command_result") |> value(callbacks)
    end
  end
