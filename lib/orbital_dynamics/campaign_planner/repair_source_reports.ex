defmodule OrbitalDynamics.CampaignPlanner.RepairSourceReports do
  ...
  def candidate_refresh_warnings(nil, _callbacks), do: nil

  def candidate_refresh_warnings(%{} = candidate_refresh, callbacks) do
    stringify_keys = Keyword.fetch!(callbacks, :stringify_keys)
  ...
  def contact_allocation_summary(%{} = candidate_refresh, callbacks) do
    candidate_refresh
    |> contact_allocation_summaries(callbacks)
    |> List.first()
  end

  def contact_allocation_summaries(candidate_refresh),
    do: contact_allocation_summaries(candidate_refresh, default_callbacks())

  def contact_allocation_summaries(nil, _callbacks), do: []

  def contact_allocation_summaries(%{} = candidate_refresh, callbacks) do
    stringify_keys = Keyword.fetch!(callbacks, :stringify_keys)
    candidate_refresh = stringify_keys.(candidate_refresh)
  ...
  def contact_allocation_station_pressure_summary(%{} = candidate_refresh, callbacks) do
    candidate_refresh
    |> contact_allocation_station_pressure_summaries(callbacks)
    |> List.first()
  end

  def contact_allocation_station_pressure_summaries(candidate_refresh),
    do:
      contact_allocation_station_pressure_summaries(
        candidate_refresh,
        default_callbacks()
      )

  def contact_allocation_station_pressure_summaries(nil, _callbacks), do: []

  def contact_allocation_station_pressure_summaries(%{} = candidate_refresh, callbacks) do
    stringify_keys = Keyword.fetch!(callbacks, :stringify_keys)
    candidate_refresh = stringify_keys.(candidate_refresh)
  ...
  def contact_allocation_reservation_conflict_summary(%{} = candidate_refresh, callbacks) do
    candidate_refresh
    |> contact_allocation_reservation_conflict_summaries(callbacks)
    |> List.first()
  end

  def contact_allocation_reservation_conflict_summaries(candidate_refresh),
    do:
      contact_allocation_reservation_conflict_summaries(
        candidate_refresh,
        default_callbacks()
      )

  def contact_allocation_reservation_conflict_summaries(nil, _callbacks), do: []

  def contact_allocation_reservation_conflict_summaries(
        %{} = candidate_refresh,
        callbacks
      ) do
    stringify_keys = Keyword.fetch!(callbacks, :stringify_keys)
  ...
  def contact_allocation_capacity_pack_summary(%{} = candidate_refresh, callbacks) do
    candidate_refresh
    |> contact_allocation_capacity_pack_summaries(callbacks)
    |> List.first()
  end

  def contact_allocation_capacity_pack_summaries(candidate_refresh),
    do:
      contact_allocation_capacity_pack_summaries(
        candidate_refresh,
        default_callbacks()
      )

  def contact_allocation_capacity_pack_summaries(nil, _callbacks), do: []

  def contact_allocation_capacity_pack_summaries(%{} = candidate_refresh, callbacks) do
    stringify_keys = Keyword.fetch!(callbacks, :stringify_keys)
    candidate_refresh = stringify_keys.(candidate_refresh)
