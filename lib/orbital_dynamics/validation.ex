defmodule OrbitalDynamics.Validation do
  ...
  def capabilities do
    %{
      ...
      validation_levels: tolerance_policy()["validation_levels"] |> Map.keys() |> Enum.sort(),
      public_facades: [
        :validation_safety_case_summary,
  ...
  """
  def reference_fixtures, do: @reference_fixtures

  @doc """
  Fetches one validation record by stable ID or implementation module.
  """
  def record(id) when is_binary(id), do: Map.fetch(@registry, id)

  def record(module) when is_atom(module) do
  ...
  def model_acceptance_report(models, opts) when is_list(models) do
    ...
    rows =
      models
      |> Enum.with_index(1)
      |> Enum.map(fn {model, rank} -> model_acceptance_row(model, rank, intended_use) end)
    ...
    %{
      ...
      "assumptions" => %{
        ...
        "blocked_validation_levels" => blocked_validation_levels(intended_use),
        "input_model_ids" => model_ids
      },
      "model_limits" => capabilities().known_limits
    }
  end
  ...
  def schema_migration_report(opts \\ []) do
    contracts = Keyword.get(opts, :contracts, OrbitalDynamics.Schema.contracts())
    ...
    %{
      ...
      "compatibility_policy_version" =>
        OrbitalDynamics.Schema.compatibility_policy()["policy_version"],
      "compatible_change_rule_count" =>
        length(OrbitalDynamics.Schema.compatibility_policy()["compatible_changes"] || []),
      "breaking_change_rule_count" =>
        length(OrbitalDynamics.Schema.compatibility_policy()["breaking_changes"] || []),
  ...
  defp normalize_intended_use(intended_use) when intended_use in @model_acceptance_intended_uses,
    do: intended_use

  defp normalize_intended_use(_intended_use), do: "analysis"

  defp model_acceptance_row(model, rank, intended_use) do
    case model_acceptance_record(model) do
      {:ok, record} ->
  ...
  defp model_acceptance_record(%{} = record) do
    record = stringify_keys(record)
  ...
  defp model_acceptance_record(id_or_module) do
    case record(id_or_module) do
      {:ok, record} -> {:ok, record}
      :error -> {:error, implementation_name(id_or_module)}
    end
  end
  ...
  defp blocked_validation_levels(intended_use) do
    declared_levels = tolerance_policy()["validation_levels"] |> Map.keys()
  ...
  """
  def reference_fixture(id) when is_binary(id), do: Map.fetch(@reference_fixtures, id)
  ...
  def verify_reference_fixture(id, observations) when is_binary(id) and is_map(observations) do
    with {:ok, fixture} <- reference_fixture(id) do
  ...
  """
  def artifact_observations("accepted_planning_state.v1", artifact) when is_map(artifact) do
  ...
  def artifact_observations("campaign_plan.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("result_artifact.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("campaign_request_lint.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("candidate_rejection_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("candidate_diff_row.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("environment_model_capability.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("environment_provider_capability.v1", artifact)
  ...
  def artifact_observations("branch_comparison_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("optimizer_contract.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("invalidated_candidate.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("candidate_diff_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    ...
    semantic_change_reasons =
      candidate_diff_semantic_change_reasons(new_candidates, invalidated_candidates)
  ...
  def artifact_observations("refresh_budget_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("execution_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("freshness_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("manifest_field_reference.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("study_manifest_lint.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("approval_requirement.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("policy_decision.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("proposed_contact.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("policy_bundle.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("subsystem_model_capability.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("realized_activity.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("plan_delta.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("candidate_activity.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("contact_intent.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    model_limits = list_values(artifact, "model_limits")
  ...
  def artifact_observations("refreshed_window.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("source_window_lineage.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("spacecraft_state_estimate.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("realized_state_snapshot.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("remaining_horizon.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("maneuver_execution_delta.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("maneuver_recommendation.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    delta_v = list_values(artifact, "delta_v_km_s")
    model_limits = list_values(artifact, "model_limits")
  ...
  def artifact_observations("backend_acceptance_policy.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("validation_tolerance_policy.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("validation_record.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("validation_reference_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("validation_check.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("timeline_diff_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("timeline_diff_summary.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("timeline_publication_summary.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("cadence_import_manifest.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("command_window_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("constraint_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("operational_timeline_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("timeline_activity_precondition_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("timeline_integrity_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("timeline_dependency_impact_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("schema_validation_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("schema_validation_batch_report.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("schema_migration_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("operator_review_package.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("timeline_transition_application_report.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("timeline_transition_application_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
    ...
    %{
      ...
      "source_activity_count" => Map.get(artifact, "source_activity_count"),
      "replacement_activity_count" => Map.get(artifact, "replacement_activity_count"),
      "application_count" => Map.get(artifact, "application_count"),
      "selected_activity_count" => Map.get(artifact, "selected_activity_count"),
      "review_required_count" => Map.get(artifact, "review_required_count"),
      "preserved_source_count" => Map.get(artifact, "preserved_source_count"),
      "recorded_replacement_count" => Map.get(artifact, "recorded_replacement_count"),
      ...
      "approval_transition_category_counts" =>
        Map.get(artifact, "approval_transition_category_counts"),
      "row_derived_application_status_counts" =>
        count_rows_by_value(review_applications, "application_status"),
      "row_derived_transition_decision_counts" =>
        count_rows_by_value(review_applications, "transition_decision"),
      "row_derived_required_operator_action_counts" =>
        count_rows_by_value(review_applications, "required_operator_action"),
      "row_derived_status_transition_category_counts" =>
        nested_row_value_counts(review_applications, ["status_transition", "transition_category"]),
      "row_derived_approval_transition_category_counts" =>
        nested_row_value_counts(
          review_applications,
      ...
      "selected_timeline_keys" =>
        artifact
        |> list_values("selected_timeline_ids")
        |> Enum.join("|"),
      "review_activity_keys" =>
        artifact
        |> list_values("review_activity_ids")
      ...
      "review_timeline_keys" =>
        ...
        |> list_values("review_timeline_ids")
        |> Enum.join("|"),
      "withheld_review_timeline_keys" =>
        artifact
        |> list_values("withheld_review_timeline_ids")
        |> Enum.join("|"),
      "execution_boundary" => get_in(artifact, ["assumptions", "execution_boundary"]),
      "operator_authority" => get_in(artifact, ["assumptions", "operator_authority"]),
      "model_limit_count" => count(artifact, "model_limits")
  ...
  def artifact_observations("timeline_activity_state.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("timeline_activity_lifecycle_state.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("timeline_activity_status_state.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("timeline_activity_approval_state.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("timeline_lifecycle_state_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
    review_rows = map_rows(artifact, "review_rows")
  ...
  def artifact_observations("timeline_preservation_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("timeline_preservation_status.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    model_limits = list_values(artifact, "model_limits")
  ...
  def artifact_observations("timeline_feedback_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("contact_allocation_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations(
    ...
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
    request_rows = map_rows(artifact, "provider_reservation_request_rows")
    review_rows = map_rows(artifact, "provider_reservation_review_rows")
    no_request_rows = provider_reservation_no_request_rows(rows, request_rows, review_rows)
    ...
    %{
      ...
      "provider_reservation_candidate_contact_count" =>
        Map.get(artifact, "provider_reservation_candidate_contact_count"),
      "row_derived_provider_reservation_candidate_contact_count" =>
        length(request_rows) + length(review_rows),
      "provider_reservation_request_contact_count" =>
        Map.get(artifact, "provider_reservation_request_contact_count"),
      "row_derived_provider_reservation_request_contact_count" => length(request_rows),
      "provider_reservation_review_contact_count" =>
        Map.get(artifact, "provider_reservation_review_contact_count"),
      "row_derived_provider_reservation_review_contact_count" => length(review_rows),
      "provider_reservation_no_request_contact_count" =>
        Map.get(artifact, "provider_reservation_no_request_contact_count"),
      "row_derived_provider_reservation_no_request_contact_count" => length(no_request_rows),
      "provider_reservation_request_status" =>
        Map.get(artifact, "provider_reservation_request_status"),
      "provider_reservation_request_contact_ids" =>
        Map.get(artifact, "provider_reservation_request_contact_ids"),
      "provider_reservation_request_contact_keys" =>
        artifact
        |> list_values("provider_reservation_request_contact_ids")
        |> Enum.join("|"),
      "provider_reservation_review_contact_ids" =>
        Map.get(artifact, "provider_reservation_review_contact_ids"),
      "provider_reservation_review_contact_keys" =>
        artifact
        |> list_values("provider_reservation_review_contact_ids")
        |> Enum.join("|"),
      "provider_reservation_no_request_contact_ids" =>
        Map.get(artifact, "provider_reservation_no_request_contact_ids"),
      "provider_reservation_no_request_contact_keys" =>
        artifact
        |> list_values("provider_reservation_no_request_contact_ids")
        |> Enum.join("|"),
      "provider_reservation_no_request_contact_ids_by_direction" =>
        Map.get(artifact, "provider_reservation_no_request_contact_ids_by_direction"),
      "row_derived_provider_reservation_no_request_contact_ids_by_direction" =>
        no_request_rows
      ...
      "provider_reservation_review_contact_ids_by_ground_station_id" =>
        Map.get(artifact, "provider_reservation_review_contact_ids_by_ground_station_id"),
      "provider_reservation_request_contact_ids_by_match_status" =>
        Map.get(artifact, "provider_reservation_request_contact_ids_by_match_status"),
      "provider_reservation_review_contact_ids_by_match_status" =>
        Map.get(artifact, "provider_reservation_review_contact_ids_by_match_status"),
      "provider_reservation_request_ids_by_match_status" =>
        Map.get(artifact, "provider_reservation_request_ids_by_match_status"),
      "provider_reservation_review_ids_by_match_status" =>
        Map.get(artifact, "provider_reservation_review_ids_by_match_status"),
      "execution_boundary" => get_in(artifact, ["assumptions", "execution_boundary"]),
      "provider_reservation_execution" =>
        get_in(artifact, ["assumptions", "provider_reservation_execution"]),
      "operator_authority" => get_in(artifact, ["assumptions", "operator_authority"])
    }
  end

  def artifact_observations("contact_filter_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("contact_contention_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("contact_contention_resolution_report.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("link_capacity_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("monte_carlo_reproducibility_report.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("pareto_frontier_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("resource_projection_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("resource_projection_flow_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
    projected_resources = map_rows(artifact, "projected_resources")
  ...
  def artifact_observations("resource_summary.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("station_calendar_provider.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("resource_filter_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    suppressed_candidates = map_rows(artifact, "suppressed_candidates")
    invalid_summary_rows = map_rows(artifact, "invalid_resource_summary_inputs")
  ...
  def artifact_observations("station_calendar_precedence_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
    model_limits = list_values(artifact, "model_limits")
  ...
  def artifact_observations("objective_satisfaction_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("objective_tradeoff_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("score_term_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("ranking_comparison_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("strategy_branch.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("strategy_recommendation.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("study_benchmark.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("operational_readiness_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("quality_gate_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("operational_quality_gate_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("operational_quality_gate_import_readiness_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("operational_quality_gate_unavailable_resource_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("operational_quality_gate_operator_training_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("operational_quality_gate_schema_validation_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("provider_counteroffer_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
  ...
  def artifact_observations("provider_counteroffer_review_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
    rows = map_rows(artifact, "rows")
    review_rows = map_rows(artifact, "review_rows")
  ...
  def artifact_observations("provider_counteroffer_import_readiness_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("provider_counteroffer_plan_impact_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("station_calendar_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("station_reservation_report.v1", artifact) when is_map(artifact) do
    artifact = stringify_keys(artifact)
  ...
  def artifact_observations("validation_safety_case_summary.v1", artifact)
      when is_map(artifact) do
    artifact = stringify_keys(artifact)
    ...
    reports =
      ...
      |> Enum.map(fn id ->
        case verify_reference_fixture(id, Map.get(observations_by_fixture, id, %{})) do
          ...
          {:error, reason} ->
            reference_fixture_error_report(id, reason)
        end
      end)
  ...
  defp reference_fixture_error_report(id, reason) do
    {:ok, fixture} = reference_fixture(id)
  ...
  defp validation_ids(assumptions) do
    []
    |> maybe_add_propagator(
      Map.get(assumptions, :propagator) || Map.get(assumptions, "propagator")
    )
    |> add_outputs(Map.get(assumptions, :outputs) || Map.get(assumptions, "outputs") || [])
  end

  defp maybe_add_propagator(ids, propagator) do
  ...
  defp add_outputs(ids, outputs) when is_list(outputs) do
    output_ids =
      outputs
  ...
  defp add_outputs(ids, _outputs), do: ids
  ...
  defp count_grouped_values(values) when is_map(values) do
    values
    |> Map.values()
    |> Enum.map(fn
      grouped_values when is_list(grouped_values) -> length(grouped_values)
      _grouped_values -> 0
    end)
    |> Enum.sum()
  end

  defp count_grouped_values(_values), do: 0
  ...
  defp provider_reservation_no_request_rows(rows, request_rows, review_rows) do
    candidate_contact_ids =
      (request_rows ++ review_rows)
      |> Enum.map(&Map.get(&1, "contact_id"))
      |> MapSet.new()

    Enum.reject(rows, &(Map.get(&1, "contact_id") in candidate_contact_ids))
  end
  ...
  defp candidate_diff_semantic_change_reasons(new_candidates, invalidated_candidates) do
