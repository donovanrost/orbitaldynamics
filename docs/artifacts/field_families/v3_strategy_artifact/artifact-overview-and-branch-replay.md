...
Collection-latency objective labels use one cross-ingress alias contract: `collection_latency`, `collection_downlink_latency`, `data_latency`, `downlink_latency`, `max_collection_latency`, `collection_latency_limit`, `delivery_latency`, `delivery_latency_limit`, `max_delivery_latency`, and `required_delivery_latency` are equivalent at standalone CandidateRefresh, mission-objective, objective-satisfaction replay, and objective-gap summary boundaries. Report-derived V3 events emit canonical `objective_type: collection_latency`; the original normalized label remains on the source report and in objective-gap type counts, with the source report path and trust boundary carried into the decision evidence.
