...
| 20 | **Cadence boundary — bounded L5 producer-side slice.** Contract: very broad. Fidelity: artifact handoff and no-write conformance, not a live Cadence integration. | [`CadenceImport`](../../lib/orbital_dynamics/cadence_import.ex) and [`OperatorReview`](../../lib/orbital_dynamics/operator_review.ex) cover extensive typed handoff. Synchronous and opt-in single-deadline monitored-worker dry-runs preserve V3 identity/authority, acknowledgement, and idempotency semantics; separate callback controllers plus independent guardians contain direct-worker failure, timeout, caller cancellation, and controller shutdown. | Downstream Cadence-owned acceptance across supported V1/V2/V3 and operational-feedback contracts remains open. The bounded path is not a malicious-code/descendant/ambient-side-effect sandbox and grants no write authority. | Depends on 12-15, 17, 22. Next slice is consumer-owned evidence against the real Cadence boundary, including volume and rejection semantics; adapter ownership remains an external decision. |
