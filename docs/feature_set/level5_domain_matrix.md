...
| 3 | **Propagation/force models — L2.** Contract: medium. Fidelity: educational and separated. | [`TwoBody`](../../lib/orbital_dynamics/propagators/two_body.ex), [`J2`](../../lib/orbital_dynamics/propagators/j2.ex), and [`TwoBodyDrag`](../../lib/orbital_dynamics/propagators/two_body_drag.ex) are separate; all cataloged propagators are educational. | One LEO path combining J2 and drag that meets declared state-error tolerance over the planning horizon. | Depends on 4 and 18; feeds 5. **W2-A:** opt-in scalar J2-plus-drag propagator with no default change. **Decision:** horizon and error envelope. |
...
| 18 | **Validation/verification — L3.** Contract: very broad. Fidelity: strong internal artifacts, weak external numerics. | [`Validation`](../../lib/orbital_dynamics/validation.ex) and the 202-fixture rollup are extensive, while catalog validation levels remain educational/analysis for propagators/events. | One external, tolerance-backed LEO truth case covering propagated state and access/eclipse boundaries for the declared horizon. | Depends on 1, 3-5, 17. **W3-C:** one Orekit/GMAT/Tudat-derived truth bundle; promote only its exact model combination. **Decision:** reference tool/tolerances/approver. |
