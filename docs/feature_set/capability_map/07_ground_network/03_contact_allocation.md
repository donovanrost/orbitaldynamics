...
Station-reservation-conflict recommendation rows separately preserve canonical contact, source-activity, ground-station, and reservation identity with their existing expiration classification across the same four copies. Executable validation rejects missing or stale derived conflict routing while paired legacy omission remains compatible; this evidence cannot accept, modify, or create a reservation.
