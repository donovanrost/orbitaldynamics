...
Provider-pressure events, risks, and branch provenance also retain the selected contact's source-window identity and normalized start/end times. Branch-comparison window fields remain a separate versioned contract concern rather than being emitted ad hoc.
