...
Station-reservation-hold recommendation rows separately preserve canonical hold and contact identities; import-readiness summary model, source, and source artifact type; and expiration classification across the same four copies. Executable validation rejects missing or stale derived hold routing or summary identity while paired legacy omission remains compatible; the evidence performs no provider or Cadence write and cannot accept a reservation or grant operator authority.
