...
Contact-intent recommendation rows likewise preserve their canonical candidate contact, source-activity, ground-station, source-window, timeline, policy-bundle, station-calendar entry, calendar-provider, and provider-entry IDs; timing bounds; required/planned contact-count and downlink-volume demand; and approval, required-action, Cadence-import, invalid-import flag/reason, activity-validity flag, gate-status, policy-classification, station-availability, contention-status, calendar-direction, calendar-status, and calendar trust-boundary context from source risks through operator review, direct selected Cadence import, and review-derived Cadence import. Executable validation rejects missing or stale derived identity, timing, demand, or review/import/policy/station context while legacy source/derived pairs may omit the optional context together. A valid activity input retains `false` without fabricating an invalidity reason.
