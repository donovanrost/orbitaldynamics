...
Contact-contention-resolution recommendation rows separately preserve canonical risk type plus contact, selected-contact, scenario, spacecraft, ground-station, source-activity, and source-window identity across the same four copies. Executable validation rejects missing or stale derived resolution routing while paired legacy omission remains compatible; selected-contact identity records the source decision but does not execute it or mutate a schedule.
