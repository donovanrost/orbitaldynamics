...
Contact-intent recommendation rows likewise preserve their canonical candidate contact, source-activity, ground-station, source-window, timeline, and policy-bundle IDs; timing bounds; required/planned contact-count and downlink-volume demand; and approval, required-action, Cadence-import, gate-status, and policy-classification context from source risks through operator review, direct selected Cadence import, and review-derived Cadence import. Executable validation rejects missing or stale derived identity, timing, demand, or review/import/policy context while legacy source/derived pairs may omit the optional context together.
