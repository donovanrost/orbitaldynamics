...
Contact-contention-resolution recommendation rows separately preserve canonical risk type; contact, selected-contact, scenario, spacecraft, ground-station, source-activity, and source-window identity; required/planned contact and downlink demand; start/end bounds; selected priority source; selection reason/rule; override count/contact IDs; review status; and demand/completion source, feedback source/scope, trust-boundary, and derivation-reason provenance across the same four copies. Executable validation rejects missing or stale derived resolution routing, demand, timing, selection/review state, or provenance while paired legacy omission remains compatible; selected-contact and override evidence record the source decision but do not execute it or mutate a schedule.
Contact-contention recommendation rows separately preserve canonical risk type; contact, scenario, spacecraft, ground-station, source-activity, source-window, and contention-group identity; required/planned contact and downlink demand; start/end bounds; resource scope and contending-contact identity; required operator action; approval status; action reason; and demand/completion source, feedback source/scope, trust-boundary, and derivation-reason provenance across the same four copies. Executable validation rejects missing or stale derived contention routing, demand, timing, operational review state, or provenance while paired legacy omission remains compatible; contention and review evidence remain descriptive and cannot grant operator authority, reserve station time, or mutate a schedule.
Contact-filter recommendation rows separately preserve canonical risk type; contact, scenario, spacecraft, ground-station, source-activity, and source-window identity; required/planned contact and downlink demand; start/end bounds; suppression reason and review status; reservation identity, owner, status, and match; calendar-entry identity/status; and demand/completion source, feedback source/scope, trust-boundary, and derivation-reason provenance across the same four copies. Executable validation rejects missing or stale derived filter routing, demand, timing, reservation state, or provenance while paired legacy omission remains compatible; the evidence cannot alter the source filter decision, accept a reservation, grant operator authority, or mutate a schedule.
