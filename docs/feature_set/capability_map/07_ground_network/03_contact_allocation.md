...
Contact-filter recommendation rows separately preserve canonical risk type; contact, scenario, spacecraft, ground-station, source-activity, and source-window identity; required/planned contact and downlink demand; and start/end bounds across the same four copies. Executable validation rejects missing or stale derived filter routing, demand, or timing while paired legacy omission remains compatible; the evidence cannot alter the source filter decision, reserve station time, or mutate a schedule.
