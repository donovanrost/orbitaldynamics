...
Contact-filter recommendation rows separately preserve canonical risk type; contact, scenario, spacecraft, ground-station, source-activity, and source-window identity; required/planned contact and downlink demand; start/end bounds; suppression reason and review status; reservation identity, owner, status, and match; and calendar-entry identity/status across the same four copies. Executable validation rejects missing or stale derived filter routing, demand, timing, or reservation state while paired legacy omission remains compatible; the evidence cannot alter the source filter decision, accept a reservation, grant operator authority, or mutate a schedule.
