...
Station-reservation-conflict recommendation rows separately preserve canonical contact, source-activity, ground-station, and reservation identity plus owner, reservation status, match status, expiration deadline, and expiration classification across the same four copies. Executable validation rejects missing or stale derived conflict routing or state/deadline context while paired legacy omission remains compatible; this evidence cannot accept, modify, or create a reservation.
