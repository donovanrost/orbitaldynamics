...
The same rows expose row-derived total source-window, bounded-window, and untimed-window counts plus canonical untimed source-window IDs. A row-derived timing-coverage status classifies the evidence as `complete`, `partial`, or `untimed`. Executable validation fixes those summaries and status to the authoritative ID/bounds lists, so timing coverage cannot drift independently in review or import artifacts. When a source copy supplies any coverage field, operator-review and Cadence rows must preserve it; legacy source/derived pairs may continue omitting the optional fields together.
