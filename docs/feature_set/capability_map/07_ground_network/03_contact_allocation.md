...
Contact-filter recommendation rows separately preserve canonical risk type plus contact, scenario, spacecraft, ground-station, source-activity, and source-window identity across the same four copies. Executable validation rejects missing or stale derived filter routing while paired legacy omission remains compatible; the routing evidence cannot alter the source filter decision, reserve station time, or mutate a schedule.
