...
Provider-pressure events, risks, and branch provenance also retain the selected contact's source-window identity and normalized start/end times. Branch comparison derives canonical source-window IDs plus the earliest event start and latest event end, and preserves that versioned context through operator-review and Cadence-import comparison rows.
When a source comparison or strategy review supplies that window context, executable handoff validation requires the derived operator-review or Cadence-import row to preserve it and rejects stale copies; legacy source and derived rows remain valid when both omit the optional fields.
Selected provider-review rows also retain their reservation expiration deadline and classification. An `expired` or `missing` classification therefore contributes the existing reservation-expiration pressure penalty beside the provider-request penalty, but aggregate expiration counts and routes remain provenance-only and cannot independently create planner effects.
