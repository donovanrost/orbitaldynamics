...
Selected provider-review rows also retain their reservation expiration deadline and classification. Candidate-source provider-review replay now reattaches the same classification only when the review contact appears in an exact expiration-status route. An `expired` or `missing` classification therefore contributes the existing reservation-expiration pressure penalty beside the provider-request penalty, while `active`, unrelated, and aggregate-only expiration evidence remain neutral and cannot independently create planner effects.
