...
Station-reservation-hold recommendation rows separately preserve canonical hold and contact identities; import status; import-readiness summary model, source, source artifact type, status, classification, and nonnegative hold count; and expiration classification across the same four copies. Executable validation rejects missing or stale derived hold routing, summary identity, or summary state while paired legacy omission remains compatible; the evidence performs no provider or Cadence write and cannot accept a reservation or grant operator authority.
