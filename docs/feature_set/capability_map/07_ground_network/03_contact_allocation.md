...
Campaign-planner provider-pressure branches retain the selected contact row's scalar and list-valued reservation IDs through the event, risk indicator, branch metadata, and branch-comparison reservation identity. This identity preservation does not reserve provider time, mutate schedules, or turn aggregate reservation routes into planner effects.
