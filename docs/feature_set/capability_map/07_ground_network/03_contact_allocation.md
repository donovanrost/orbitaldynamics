...
The same rows expose row-derived total source-window, bounded-window, and untimed-window counts plus canonical untimed source-window IDs. Executable validation fixes those summaries to the authoritative ID/bounds lists, so timing coverage cannot drift independently in review or import artifacts.
