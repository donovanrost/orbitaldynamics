...
- CandidateRefresh accepts direct, accepted-state, mission-state, and result-artifact-wrapped `station_reservation_review_summary.v1` handoffs as compact station-reservation provenance, preserving review rows, source paths, reservation counts, owner/status maps, expiration evidence, and branch-local replay pressure without rerunning reservation matching or mutating station calendars.
...
- `StationCalendar.precedence_summary/1`/`3` and `OrbitalDynamics.station_calendar_precedence_summary/1`/`3` publish the validated `station_calendar_precedence_summary.v1` contract with applied/overlap availability counts, contact ID maps, and reserved-under-higher-precedence routing from `station_calendar_report.v1` affected rows without provider reservation or schedule mutation.
