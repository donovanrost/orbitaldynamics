...
- Derived score-term pressure recommendation rows preserve source-exact risk,
  ...
  derivation reasons preserve the scoring lineage. Missing or stale derived
  routing, timing, demand, geometry, score state, or provenance is rejected while
...
- Derived objective-tradeoff pressure recommendation rows preserve source-exact
  ...
  target geometry. Feedback source/scope, trust boundary, and derivation
  reasons remain exact provenance. Missing or stale derived evidence is
  rejected while paired legacy omission remains compatible; the evidence
  cannot approve an import, write to Cadence, or execute a schedule.
...
- Derived objective-satisfaction pressure recommendation rows preserve
  ...
  remain exact too, along with source-activity IDs, feedback source/scope, trust
  boundary, and derivation reasons. Downlink objectives also retain their
  latency flag, station, required/planned contacts and volume, and latency
  bounds plus missed activity, realized/contact outcome, and demand/completion
  sources. Candidate windows, allowed scenarios, spacecraft constraints, and
  coverage objective remain exact for target evidence. Missing or stale derived
  identity/status/timing/demand/geometry/quality/provenance/constraint context is
