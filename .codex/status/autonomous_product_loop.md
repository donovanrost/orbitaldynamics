...
Current slice:
Enforce source-exact provider-calendar provider identity context.

Status:
Verified; publish pending.

Selection evidence:
- The selected branch event supplies provider ID `partner_calendar`, but the
...
- The canonical aggregator exists; review/import schemas and exact-copy
  validation omit the stable provider identity list.
...
- Reject missing or stale provider IDs when source risks supply the list;
...
- provider-identity mutation/schema proofs, docs, exports, and ledger

Verification:
- Focused handoff/schema proof: `48 passed`.
- Contact-allocation regression: `213 passed`.
- Golden artifact regression: `12 passed`.
- Schema lint: `155` artifacts, `0` errors, `0` warnings.
- Full suite: `3921 passed`.
...
Review:
- Passive projection now retains the source provider ID without changing risk
  scoring or planning behavior.
...
- `6a286827` Validate provider calendar contention entries (`3920 passed`).
...
Next candidate:
Assess source-exact provider-calendar provider-entry identity context.
