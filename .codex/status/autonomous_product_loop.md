...
Completed slice:
Expose composed timeline-diff replay branch flags on
`CandidateRefresh.source_report_summary/1`.

Why this slice:
The public capability catalog advertises
`source_report_timeline_diff_branch_replay_summary`. The dedicated
`timeline_diff_replay_summary/1` helper already derives branch-local
timeline-diff, duplicate-identity, removed-activity, changed-activity,
activity-routing, and operator-review pressure, but the main
`source_report_summary/1` surface only exposed the underlying timeline-diff
rollups. Adapter and operator-review callers can now inspect the composed
timeline-diff branch flags from the same source-report summary surface.

Level 6 pillar:
Branch-local candidate refresh depth and timeline-diff replay semantics.

What changed:
- `CandidateRefresh.source_report_summary/1` now exposes timeline-diff
  branch-local replay flags for timeline-diff, duplicate-identity,
  removed-activity, changed-activity, activity-routing, and operator-review
  pressure.
- `timeline_diff_replay_summary/1` now uses a private summary builder, so the
...
- Candidate-refresh regression coverage proves the source-report summary
  carries the composed timeline-diff branch flags for raw provenance and
  compact `timeline_diff_report` provenance.

Verification:
- `mix test test/orbital_dynamics/candidate_refresh_test.exs:18460` passed, 1
  test.
...
Next candidate:
Candidate-refresh timeline-integrity replay branch summary routing. The public
capability catalog advertises `source_report_timeline_integrity_branch_replay_summary`,
and `timeline_integrity_replay_summary/1` already derives branch-local
timeline-integrity pressure flags that are not yet projected through
`CandidateRefresh.source_report_summary/1`.
