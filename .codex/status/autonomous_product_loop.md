...
Current slice:
Blocked link-capacity pressure affects V3 recommendation selection.

Status:
Implemented, parent-reviewed, verified, and published locally.
Behavior commit: `2c99fcd`.
...
Tests/checks run:
- `mix test test/orbital_dynamics/campaign_planner_test.exs:8587`
...
- Final `mix test`: `3334 passed`.
- `git diff --check`

Behavior changed:
Blocked link-capacity evidence now affects V3 branch recommendation selection
by default before review/import handoff. The V3 default approval policy includes
the semantic `link_capacity_blocked` alias, branch-local
`downlink_completion_gap` link-capacity pressure carries link/downlink status
fields into risk indicators, and the shared fallback matcher treats blocked
link-capacity pressure as blocked without making ordinary shortfall
link-capacity pressure blocked by default.

Level 6 pillar advanced:
Planner-visible communications gating. Blocked link-capacity evidence should be
...
`2c99fcd` Block link capacity recommendations by default.
...
Notes:
- Selection note: live search found link-capacity rows are converted to
  branch-local `downlink_completion_gap` events with
  `feedback_scope: link_capacity` and already feed
  `link_capacity_pressure_penalty`. Contact-allocation policy blocks and
  contact-intent policy blocks are covered, but default fallback matching has no
  link-capacity semantic alias. Docs read:
  ...
  link-capacity pressure is classified `blocked_by_policy` by default and is
  skipped when a selectable branch exists, ordinary link-capacity shortfall
  pressure remains reviewable/selectable unless configured otherwise, focused
  and full tests pass, parent review is recorded, and behavior plus ledger
  commits are pushed.
- Parent review notes: implementation kept the new default policy alias
  ...
  high-value blocked link-capacity branch is skipped in favor of baseline while
  ordinary shortfall link-capacity pressure remains selectable with
