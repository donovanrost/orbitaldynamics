...
Current slice:
Timeline-activity lifecycle-state replay reads and labels V3 branch
...
counts, paths, summary model/schema counts, transition/status/approval/import/
action maps, activity/timeline routing, protection evidence, trust-boundary
metadata, invalid-input evidence, and branch-local pressure booleans while
labeling their `source` and replay scope as candidate-source summary metadata.
Empty branch families fall back to provenance and keep existing provenance-only
...
Tests run:
- `mix test test/orbital_dynamics/candidate_refresh_test.exs:13221 test/orbital_dynamics/candidate_refresh_test.exs:13449 test/orbital_dynamics/candidate_refresh_test.exs:13464 test/orbital_dynamics/candidate_refresh_test.exs:13586 test/orbital_dynamics/candidate_refresh_test.exs:13623 test/orbital_dynamics/candidate_refresh_test.exs:13665 --trace --seed 0`
  passed activity lifecycle-state review/import aggregation,
...
- `mix test test/orbital_dynamics/campaign_planner_test.exs:26320 --trace --seed 0`
  passed the strategy branch refresh caller that passes direct
  candidate-source maps into timeline activity lifecycle-state replay.
- `git diff --check` passed.

Docs/artifacts changed:
- `docs/artifacts/field_families/candidate_refresh_artifact.md` now documents
  the V3 activity lifecycle-state branch candidate-source summary preference,
  source and replay-scope labels, partial-family precedence, and provenance
  fallback. `docs/artifacts/field_families/mission_activities.md` carries the
  same companion field-family note. No schema exports or checked-in artifacts
...
before choosing another gap. CandidateRefresh replay helpers still include
...
Notes:
This slice intentionally does not apply lifecycle-state changes, mutate
