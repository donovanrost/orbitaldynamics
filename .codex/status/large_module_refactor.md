...
Outcome:
Removed the 11-entry evidence callback bag and all owner trampolines. Shared
primitive validation is direct; stable sorting, nonnegative map sum, and error
construction remain exact local behavior; resource and timeline-publication
validators are explicit function boundaries. Two now-unused Schema helpers were
removed. `schema.ex` fell from 11,860 to 11,827 lines and the evidence owner from
531 to 470.
...
- compile-connected xref showed only the primitive dependency
...
Operational-readiness-evidence callback-bag collapse; publication commit pending.
