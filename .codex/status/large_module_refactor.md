...
`Schema.RefreshBudgetReportContracts.validate/3`. Remove the pure facade
delegate while preserving the pipe position and issue ordering.

Why this slice:
`Schema` remains a named 7,983-line production hotspot. The established
RefreshBudgetReport owner already exposes the exact `/3` implementation, and
the facade delegate has exactly one caller, so this is a bounded
candidate-refresh report ownership cleanup.

Current coupling/problem:
The standalone refresh-budget validator still routes through a private facade
callback even though `RefreshBudgetReportContracts` owns the full
implementation.

Public facade to preserve:
All `OrbitalDynamics.Schema` public functions, exact validation issue ordering,
paths and messages, JSON Schema output, checked-in export bytes, and
refresh-budget artifact behavior.

Likely extraction target:
Existing `OrbitalDynamics.Schema.RefreshBudgetReportContracts.validate/3`.
...
Definition of done:
The standalone refresh-budget pipe calls the established owner in the same
...
- Source baseline: `validate_refresh_budget_report/3` appears exactly once as
  ...
  its pure facade definition; the established RefreshBudgetReport owner exposes
  the exact `validate/3` implementation.
