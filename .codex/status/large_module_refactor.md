...
Current slice:
Schema remaining-horizon callback ownership handoff.

Status:
Published as `d9717806`.
...
- Source proof against selection commit `56212ea6`: the standalone
  remaining-horizon pipe retains its `require_fields` stage and now ends
  directly at `CandidateRefreshWindowContracts.validate_remaining_horizon/3`;
  the private facade delegate is absent and no other facade call site remains.
...
- Independent bounded review against selection commit `56212ea6` was clean:
...
Outcome:
The standalone remaining-horizon contract pipe now calls the established owner
directly and the pure facade delegate is gone. `schema.ex` decreased from 7,988
to 7,983 lines.
...
Next candidate:
Map the adjacent single-call `validate_refresh_budget_report/3` facade
delegate. Its established `RefreshBudgetReportContracts.validate/3` owner is
unchanged and the standalone `refresh_budget_report.v1` pipe is its only
