...
`Schema.ResourceFilterRegistryContracts`.

Why this slice:
The summary directly nests the filter report, forming one cohesive suppression
family with dedicated resource/filter and export coverage.

Current coupling/problem:
Declarative resource-filter contract data remains embedded in the large
...
`OrbitalDynamics.Schema.ResourceFilterRegistryContracts.contracts/0`.

Likely files:
- `.codex/status/large_module_refactor.md`
- `lib/orbital_dynamics/schema.ex`
- `lib/orbital_dynamics/schema/resource_filter_registry_contracts.ex`
...
None. Registry contents, resource-filter validation, and generated schemas retain
the baseline fingerprint.

Tests run:
- `mix compile --warnings-as-errors` passed.
- Resource contracts, filter reports, communications report fixtures, fixture
  visibility, registry capability, and schema export tests passed: 21 tests.
...
Notes:
- `schema.ex` decreased from 17,037 to 16,968 lines.
- `ResourceFilterRegistryContracts` is 78 lines.
